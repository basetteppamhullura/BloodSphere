...
export const BloodBankHomePage: React.FC = () => {
  ...
  const availableCount = bloodUnitsList.filter(
    u => u.status === 'APPROVED' || u.status === 'STORED' || u.status === 'COLLECTED'
  ).length;
  ...
  const expiringSoonCount = bloodUnitsList.filter(
    u => u.status !== 'EXPIRED' && u.status !== 'ISSUED' && u.expiryDate >= todayStr && u.expiryDate <= fiveDaysFromNow
  ).length;
  ...
  const lowStockGroupsList = allGroups.filter(bg => {
    ...
    const totalAvail: number = Number(Object.values(groupStock).reduce<number>((acc: number, comp: any) => acc + Number(comp?.available || 0), 0));
    return totalAvail < 5;
  });
  ...
  const todayIssuedCount = activityLogs.filter(
    l => l.action.toLowerCase().includes('issued') && (l.date === todayStr || l.date.includes('2026'))
  ).length;
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-16 animate-in fade-in">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-5 py-3 rounded-2xl bg-white text-emerald-800 hover:bg-emerald-50 font-black text-xs shadow-lg flex items-center gap-2 transition-all hover:scale-105 cursor-pointer"
            >
            ...
            <button
              ...
              className="px-4 py-3 rounded-2xl bg-emerald-600/60 hover:bg-emerald-600 text-white font-bold text-xs border border-white/20 flex items-center gap-1.5 transition-all cursor-pointer"
            >
            ...
            <button
              ...
              className="px-3.5 py-3 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs border border-white/20 flex items-center gap-1.5 transition-all cursor-pointer"
              title="Logout"
            >
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          {/* Card 12: Action Card */}
          <div
            onClick={() => navigate('/bloodbank/issue')}
            className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 text-white shadow-xs space-y-1 cursor-pointer hover:opacity-95 transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-3">
          ...
          <div className="pt-2 flex items-center gap-2">
            <button
              onClick={() => navigate('/bloodbank/requests')}
              className="flex-1 py-2 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-black text-xs transition-colors"
            >
              Open Requester Queue →
            </button>
            <button
              onClick={() => navigate('/bloodbank/hospital-requests')}
        ...
        <div className="p-5 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-3">
          ...
          <div className="pt-2 flex items-center gap-2">
            <button
              onClick={() => navigate('/bloodbank/inventory')}
              className="flex-1 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs transition-colors"
            >
              View Inventory Matrix →
            </button>
            <button
              onClick={() => navigate('/bloodbank/preservation')}
              className="py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs"
            >
              Vaults
            </button>
        ...
        <div className="p-5 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-3">
          ...
          <div className="pt-2 flex items-center gap-2">
            <button
              ...
              className="flex-1 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-black text-xs transition-colors"
            >
              Track Barcode Units →
            </button>
            <button
              onClick={() => navigate('/bloodbank/activity')}
              className="py-2 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs"
            >
              Audit Log
            </button>
      ...
      <div className="p-6 rounded-3xl bg-emerald-50/70 border border-emerald-200 flex flex-col md:flex-row items-center justify-between gap-4 text-emerald-950">
        ...
        <button
          onClick={() => navigate('/bloodbank/requests')}
          className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shrink-0 transition-colors cursor-pointer"
        >
