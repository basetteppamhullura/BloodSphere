...
export const LandingPage = () => {
    ...
    return (<div className="space-y-8 py-2 animate-in fade-in text-xs">
      ...
      <section className="relative overflow-hidden p-8 sm:p-12 rounded-3xl bg-gradient-to-br from-white via-sky-50/40 to-slate-50 border border-sky-100/90 shadow-xs space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6 pt-2 relative z-10">
          ...
          {/* Card 1: Donor Portal */}
          <div onClick={() => navigate('/login/donor')} role="button" tabIndex={0} onKeyDown={(e) => {
          ...
          <div onClick={() => navigate('/login/requester')} role="button" tabIndex={0} onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
          ...
          <div onClick={() => navigate('/login/hospital')} role="button" tabIndex={0} onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
          ...
          <div onClick={() => navigate('/login/bloodbank')} role="button" tabIndex={0} onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <Link to={getFindBloodPath()} className="px-6 py-3.5 rounded-2xl bg-sky-600 hover:bg-sky-500 text-white font-extrabold text-xs shadow-md shadow-sky-500/25 flex items-center gap-2 transition-all hover:scale-105 cursor-pointer">
                <Search className="w-4 h-4"/> Find Blood Now
              </Link>

              <Link to={getDonateBloodPath()} className="px-6 py-3.5 rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white font-extrabold text-xs shadow-md shadow-red-500/25 flex items-center gap-2 transition-all hover:scale-105 cursor-pointer">
                <Heart className="w-4 h-4 fill-white"/> Donate Blood
              </Link>

              <button type="button" onClick={() => setActiveEmergencyPostModal(true)} className="px-5 py-3.5 rounded-2xl bg-white hover:bg-red-50 text-red-600 font-extrabold text-xs border border-red-200 shadow-xs flex items-center gap-2 transition-all cursor-pointer">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            {shortageAlerts.map((alert, idx) => (<div key={idx} className="p-3 rounded-2xl bg-amber-50/60 border border-amber-200 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
        ...
        <div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
            {notifications.slice(0, 4).map(notif => (<div key={notif.id} className="p-3 rounded-2xl bg-sky-50/40 border border-sky-100 space-y-1">
                <div className="flex items-center justify-between">
