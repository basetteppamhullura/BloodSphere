...
import { LogIn, Building2, Droplet, AlertCircle, KeyRound, Check, Eye, EyeOff, Heart, ArrowRight, ArrowLeft, ShieldCheck, Loader2, RefreshCw, HeartPulse, Users, Shield } from 'lucide-react';
export const LoginPage = ({ initialPortal }) => {
    ...
    const getSelectedPortal = () => {
        ...
        if (path === '/login/bloodbank' || path.includes('/bloodbank'))
            return 'bloodbank';
        if (path === '/login/hospital' || path.includes('/hospital'))
            return 'hospital';
        if (path === '/login/requester' || path.includes('/requester'))
            return 'requester';
        if (path === '/login/admin' || path.includes('/admin'))
            return 'admin';
        if (path === '/login/donor' || path.includes('/donor'))
            return 'donor';
        if (params.portal && ['donor', 'requester', 'hospital', 'bloodbank', 'admin'].includes(params.portal)) {
            return params.portal;
        }
    ...
    const [activePortalTab, setActivePortalTab] = useState(selectedPortal || 'donor');
    // Sub-role toggle for Donor & Requester combined view
    const [donorRequesterSubRole, setDonorRequesterSubRole] = useState(selectedPortal === 'requester' ? 'requester' : 'donor');
    // Mode ('signin' | 'signup')
    ...
    const applyPortalCredentials = (portal) => {
        setLoginError(null);
    ...
    const handleSignInSubmit = (e) => {
        e.preventDefault();
        setLoginError(null);
        const effectiveRole = selectedPortal || activePortalTab;
        if (!email || !password) {
        ...
        setTimeout(() => {
            const res = login(email, effectiveRole, (effectiveRole === 'hospital' || effectiveRole === 'bloodbank') ? licenseNumber : undefined);
    ...
    return (<div className="relative min-h-[90vh] py-10 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-slate-50 via-sky-50/20 to-slate-50 flex flex-col justify-center items-center overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none select-none overflow-hidden">
        ...
        <svg className="absolute inset-0 w-full h-full text-sky-900/[0.04]" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" viewBox="0 0 1200 800" fill="none">
          {/* Soft Medical Waves */}
          <path d="M0,200 C300,160 500,240 800,190 C1000,160 1150,220 1200,200 L1200,800 L0,800 Z" fill="currentColor" opacity="0.25"/>

          {/* Subtle ECG Heartbeat Pulse Line */}
          <path d="M0,450 L380,450 L400,435 L415,480 L435,390 L455,470 L470,445 L485,450 L1200,450" stroke="currentColor" strokeWidth="2.5" strokeDasharray="8 6" opacity="0.8"/>
      ...
      <div className="w-full max-w-4xl space-y-8 relative z-10 animate-in fade-in duration-300">
        ...
        {/* ================================================== */}
        {!selectedPortal ? (<div className="space-y-8">
              ...
              {portalCards.map((card) => {
                ...
                return (<div key={card.id} id={`portal-card-${card.id}`} onClick={() => navigate(card.route)} role="button" tabIndex={0} onKeyDown={(e) => {
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                      ...
                      <button type="button" tabIndex={-1} className={`px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer ${card.buttonClasses}`}>
            ...
            <div className="text-center pt-3 flex flex-col sm:flex-row items-center justify-center gap-4 text-xs">
              ...
              <button type="button" onClick={() => navigate('/login/admin')} className="text-slate-500 hover:text-amber-700 font-bold hover:underline inline-flex items-center gap-1 cursor-pointer">
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between max-w-xl mx-auto px-1">
              <button type="button" onClick={() => navigate('/login')} className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:text-slate-900 bg-white hover:bg-slate-100 border border-slate-200 px-3.5 py-1.5 rounded-full shadow-2xs transition-all cursor-pointer hover:scale-102">
            ...
            <div className="max-w-xl mx-auto p-7 sm:p-9 rounded-3xl bg-white border border-slate-200/90 shadow-xl space-y-6 relative overflow-hidden transition-all">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-5">
                <div className="flex items-center gap-3.5">
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center text-xl font-bold shadow-xs ${selectedPortal === 'hospital'
                ...
                : selectedPortal === 'bloodbank'
                  ...
                  <div>
                    <h2 className="font-black text-lg sm:text-xl text-[#0F172A] leading-tight">
                      {selectedPortal === 'requester' ? (authMode === 'signin' ? 'Requester Portal Sign In' : 'Requester Account Registration') : selectedPortal === 'donor' ? (authMode === 'signin' ? 'Donor Portal Sign In' : 'Donor Account Registration') : selectedPortal === 'hospital' ? (authMode === 'signin' ? 'Hospital Portal Sign In' : 'Hospital Account Registration') : selectedPortal === 'bloodbank' ? (authMode === 'signin' ? 'Blood Bank Portal Sign In' : 'Blood Bank Account Registration') : ('Super Admin Secure Sign In')}
                    </h2>
              ...
              {currentAttempts > 0 && currentAttempts < 5 && (<div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-xs font-bold flex items-center gap-2.5 animate-in fade-in">
              ...
              {/* ERROR ALERT BANNER */}
              {loginError && (<div className="p-3.5 rounded-2xl bg-red-50 border border-red-200 text-red-700 text-xs font-bold flex items-start gap-2.5 animate-in slide-in-from-top-2">
            ...
            <form onSubmit={handleSignInSubmit} className="space-y-4 text-xs">
                    ...
                    : selectedPortal === 'bloodbank'
                      ...
                      <input type="text" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={selectedPortal === 'hospital'
                    ? 'admin@kims.edu.in'
                    : selectedPortal === 'bloodbank'
                        ...
                        : selectedPortal === 'admin'
                            ...
                            : selectedPortal === 'requester'
                                ...
                                : 'ananya.sharma@example.com'} className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                    </div>

                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-slate-700 font-extrabold block">Password *</label>
                        <button type="button" onClick={() => showToast('Password reset instructions dispatched to your registered email.')} className="text-[11px] font-bold text-slate-400 hover:text-sky-600 transition-colors cursor-pointer">
                          Forgot Password?
                        </button>
                      </div>
                      <div className="relative">
                        <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full p-3.5 pr-10 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3.5 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer" title={showPassword ? 'Hide password' : 'Show password'}>
                    ...
                    {(selectedPortal === 'hospital' || selectedPortal === 'bloodbank') && (<div className="space-y-1 animate-in fade-in">
                        ...
                        <input type="text" value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)} placeholder={selectedPortal === 'hospital' ? 'e.g. LIC-HUB-4482' : 'e.g. LIC-BB-9901'} className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all font-mono uppercase tracking-wider" required/>
                    ...
                    <button type="submit" disabled={isSubmitting} className={`w-full py-4 rounded-2xl font-black text-xs shadow-md flex items-center justify-center gap-2 transition-all cursor-pointer ${selectedPortal === 'hospital'
                    ...
                    : selectedPortal === 'bloodbank'
                        ...
                        : selectedPortal === 'admin'
                            ...
                            : selectedPortal === 'requester'
                                ...
                                : 'bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white shadow-red-500/20'} ${isSubmitting ? 'opacity-70 cursor-wait' : 'hover:scale-[1.01]'}`}>
                      {isSubmitting ? (<>
            ...
            <form onSubmit={handleVerifyOtpSubmit} className="space-y-4 text-xs animate-in fade-in">
                    ...
                    <div>
                      ...
                      <input type="text" maxLength={6} value={otpInput} onChange={(e) => setOtpInput(e.target.value)} className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-mono font-black text-center text-xl tracking-widest focus:outline-none focus:border-emerald-500 focus:bg-white" required/>
                    </div>
                    ...
                    setLoginError(null);
                      ...
                      <button type="button" onClick={handleResendOtp} disabled={isResendingOtp} className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 hover:underline cursor-pointer">
                        <RefreshCw className={`w-3 h-3 ${isResendingOtp ? 'animate-spin' : ''}`}/>
                        Resend 2FA Code
                      </button>
                    </div>

                    <button type="submit" disabled={isVerifyingOtp} className="w-full py-4 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs shadow-md shadow-emerald-500/20 flex items-center justify-center gap-2 cursor-pointer transition-all hover:scale-[1.01]">
                      {isVerifyingOtp ? (<>
                          <Loader2 className="w-4 h-4 animate-spin"/>
            ...
            <form onSubmit={handleSignUpSubmit} className="space-y-4 text-xs animate-in fade-in">
                  <div>
                    ...
                    <input type="text" value={regName} onChange={(e) => setRegName(e.target.value)} placeholder={selectedPortal === 'hospital'
                    ? 'e.g. City General Hospital'
                    : selectedPortal === 'bloodbank'
                        ? 'e.g. Red Cross Blood Center'
                        : 'e.g. Rajesh Kumar'} className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-slate-700 font-extrabold block mb-1">Official Email *</label>
                      <input type="email" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} placeholder="email@example.com" className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                    </div>

                    <div>
                      <label className="text-slate-700 font-extrabold block mb-1">Contact Phone *</label>
                      <input type="tel" value={regPhone} onChange={(e) => setRegPhone(e.target.value)} placeholder="+91 98765 43210" className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                    </div>
                  </div>

                  {/* DONOR BLOOD GROUP SELECTION */}
                  {selectedPortal === 'donor' && (<div>
                      <label className="text-slate-700 font-extrabold block mb-1">Blood Group *</label>
                      <select value={regBloodGroup} onChange={(e) => setRegBloodGroup(e.target.value)} className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all">
                        {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map((bg) => (<option key={bg} value={bg}>
                            {bg}
                          </option>))}
                      </select>
                    </div>)}
                  ...
                  {/* HOSPITAL / BLOOD BANK LICENSE REGISTRATION NUMBER */}
                  {(selectedPortal === 'hospital' || selectedPortal === 'bloodbank') && (<div>
                      <label className="text-slate-700 font-extrabold block mb-1">
                        {selectedPortal === 'hospital'
                        ? 'Hospital License Registration Number *'
                        : 'Blood Bank License Registration Number *'}
                      </label>
                      <input type="text" value={regLicense} onChange={(e) => setRegLicense(e.target.value)} placeholder={selectedPortal === 'hospital' ? 'LIC-HOSP-XXXX' : 'LIC-BB-XXXX'} className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all font-mono uppercase" required/>
                    </div>)}

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="text-slate-700 font-extrabold block mb-1">City / Region *</label>
                      <input type="text" value={regCity} onChange={(e) => setRegCity(e.target.value)} placeholder="Hubballi" className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                    </div>

                    <div>
                      <label className="text-slate-700 font-extrabold block mb-1">Password *</label>
                      <input type="password" value={regPassword} onChange={(e) => setRegPassword(e.target.value)} placeholder="••••••••" className="w-full p-3.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500 focus:bg-white transition-all" required/>
                    </div>
                  </div>

                  <button type="submit" disabled={isRegistering} className={`w-full py-4 rounded-2xl font-black text-xs shadow-md flex items-center justify-center gap-2 cursor-pointer transition-all hover:scale-[1.01] ${selectedPortal === 'hospital'
                    ? 'bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 text-white shadow-sky-500/20'
                    : selectedPortal === 'bloodbank'
                        ? 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 text-white shadow-emerald-500/20'
                        : selectedPortal === 'requester'
              ...
              {/* ================================================== */}
              {selectedPortal !== 'admin' && (<div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                  {authMode === 'signin' ? (<>
