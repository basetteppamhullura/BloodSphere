...
import { Flame, Trophy, Award, ShieldCheck, Share2, FileText, AlertCircle, MapPin, CheckCircle2, Lock, ExternalLink } from 'lucide-react';
    ...
    return (<div className="space-y-6 text-xs animate-in fade-in">
      ...
      <div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-6 shadow-sm">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-b border-sky-100 pb-6">
          ...
          <button onClick={() => showToast("Referral code copied! Share with friends to earn 100 points.")} className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-sky-50 hover:bg-sky-100 text-xs font-extrabold text-sky-900 border border-sky-200 shadow-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-100 space-y-2">
            ...
            <select value={loggedInDonor.availabilityStatus || 'AVAILABLE'} onChange={e => handleAvailabilityChange(e.target.value)} className="w-full p-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 font-extrabold text-xs focus:outline-none focus:border-sky-500 cursor-pointer">
          ...
          <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-100 space-y-2">
            ...
            <div className={`p-2.5 rounded-xl border flex items-center justify-between text-xs font-extrabold ${eligibility.isEligible ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-900'}`}>
            ...
            {!eligibility.isEligible && (<p className="text-[10px] text-amber-700 font-sans">{eligibility.reason}</p>)}
          </div>
      ...
      <div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="p-3.5 rounded-2xl bg-sky-50/50 border border-sky-100 space-y-1">
            ...
            <input type="number" value={weight} onChange={(e) => setWeight(parseFloat(e.target.value))} className="w-full bg-white p-2.5 rounded-xl text-slate-900 font-bold border border-slate-200"/>
          ...
          <div className="p-3.5 rounded-2xl bg-sky-50/50 border border-sky-100 space-y-1">
            ...
            <input type="number" step="0.1" value={hbLevel} onChange={(e) => setHbLevel(parseFloat(e.target.value))} className="w-full bg-white p-2.5 rounded-xl text-slate-900 font-bold border border-slate-200"/>
        ...
        <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-100 flex items-center justify-between text-[11px] text-slate-600">
          ...
          <a href="https://nbtc.naco.gov.in" target="_blank" rel="noreferrer" className="text-sky-600 font-bold hover:underline flex items-center gap-1">
      ...
      <div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
        ...
        <div className="space-y-3">
          {(currentUser?.donationHistory || [
            ...
            { id: "dh_2", date: "2025-11-15", location: "Rotary Regional Blood Center", units: 1, bloodGroup: loggedInDonor.bloodGroup, certificateUrl: "#" }
        ]).map((don) => (<div key={don.id} className="p-4 rounded-2xl bg-sky-50/40 border border-sky-100 flex items-center justify-between gap-4 text-xs">
              <div>
              ...
              <button onClick={() => setActiveCertificateModal(don)} className="px-3.5 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 font-extrabold border border-amber-200 flex items-center gap-1.5 shrink-0">
