...
export const GroupCirclesPage = () => {
    const { groupCircles, toggleCircleJoin, requests, showToast } = useApp();
    const { currentUser } = useAuth();
    const [showCreateModal, setShowCreateModal] = useState(false);
    const [circleName, setCircleName] = useState('Muttu\'s Family Circle');
    ...
    const handleShareRequestToCircle = (circleName) => {
    ...
    return (<div className="space-y-6 text-xs animate-in fade-in">
      ...
      <div className="p-6 rounded-3xl bg-white border border-sky-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
        ...
        <button onClick={() => setShowCreateModal(true)} className="flex items-center justify-center gap-2 px-5 py-3 rounded-2xl bg-sky-600 hover:bg-sky-500 text-white font-extrabold text-xs shadow-md shadow-sky-500/20 transition-all hover:scale-105">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {groupCircles.map((circle) => {
            ...
            return (<div key={circle.id} className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 flex flex-col justify-between hover:border-sky-300 transition-all shadow-sm">
              ...
              <div className="space-y-2 pt-2 border-t border-sky-100">
                <button onClick={() => handleShareRequestToCircle(circle.name)} className="w-full py-2 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 font-extrabold text-xs border border-amber-200 flex items-center justify-center gap-1.5">
                  <Share2 className="w-3.5 h-3.5 text-amber-600"/> Share Emergency Request
                </button>

                <button onClick={() => toggleCircleJoin(circle.id)} className={`w-full py-2.5 rounded-xl font-extrabold text-xs flex items-center justify-center gap-2 transition-all ${circle.joined
      ...
      {showCreateModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-sky-100 rounded-3xl p-6 space-y-4 text-xs shadow-xl">
            ...
            <form onSubmit={handleCreateCircleSubmit} className="space-y-3">
              <div>
                ...
                <input type="text" value={circleName} onChange={e => setCircleName(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold" required/>
              </div>

              <div>
                <label className="text-slate-700 font-bold block mb-1">Circle Category *</label>
                <select value={category} onChange={e => setCategory(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
