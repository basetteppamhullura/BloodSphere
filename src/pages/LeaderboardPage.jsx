    ...
    return (<div className="space-y-6 text-xs animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sky-100 pb-4">
        ...
        <div className="flex items-center gap-1 bg-white p-1.5 rounded-2xl border border-sky-100 font-extrabold shadow-xs">
          <button onClick={() => setTab('monthly')} className={`px-4 py-2 rounded-xl text-xs transition-all ${tab === 'monthly' ? 'bg-amber-500 text-white font-black shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}>
            🔥 August 2026 Monthly
          </button>
          <button onClick={() => setTab('allTime')} className={`px-4 py-2 rounded-xl text-xs transition-all ${tab === 'allTime' ? 'bg-amber-500 text-white font-black shadow-xs' : 'text-slate-600 hover:text-slate-900'}`}>
      ...
      <div className="rounded-3xl bg-white border border-sky-100 overflow-x-auto shadow-sm">
        <table className="w-full text-left text-xs text-slate-700">
          ...
          <tbody className="divide-y divide-sky-100 font-mono">
            ...
            return (<tr key={index} className="hover:bg-sky-50/40 transition-colors">
                  ...
                  <td className="py-4 px-5 font-black text-red-600">
                    <span className="flex items-center gap-1">
                      <Flame className="w-3.5 h-3.5 text-amber-500"/> {item.donationsCount || item.donations} Verified
                    </span>
                  </td>
