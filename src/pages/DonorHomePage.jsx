...
import { Heart, LayoutDashboard, AlertTriangle, CheckCircle2, Bell, User, LogOut, Clock, ArrowRight, ShieldCheck, MapPin, Activity } from 'lucide-react';
...
export const DonorHomePage = () => {
    ...
    const urgentRequests = requests.filter(r => (r.urgency === 'CRITICAL' || r.urgency === 'HIGH') && r.status !== 'COMPLETED' && r.status !== 'CANCELLED').slice(0, 3);
    ...
    return (<div className="max-w-6xl mx-auto space-y-6 pb-12 animate-in fade-in">
      ...
      <PortalHero
        portalLabel="Voluntary Donor Portal"
        badgePulseColor="bg-emerald-300"
        title={`Welcome back, ${currentUser?.name || 'Valued Donor'}! ❤️`}
        description="Manage your donation activity, availability, appointments and blood donation history."
        bgImage="/bloodnet-hero-bg.jpg"
        bgPosition="center right"
        gradientOverlay="linear-gradient(95deg, rgba(16, 37, 66, 0.90) 0%, rgba(37, 99, 235, 0.78) 50%, rgba(14, 165, 233, 0.50) 100%)"
        decorativeIcon={Heart}
        actions={
          <>
            <button onClick={() => navigate('/donor/dashboard')} className="px-6 py-3.5 rounded-2xl bg-white text-[#2563EB] hover:bg-sky-50 font-black text-sm shadow-md flex items-center gap-2 transition-all hover:scale-105 cursor-pointer">
      ...
      <div className="p-3 rounded-2xl bg-white border border-sky-100 shadow-xs flex flex-wrap items-center justify-between gap-3 text-xs font-extrabold">
        <div className="flex flex-wrap items-center gap-2">
          <Link to="/donor/home" className="px-4 py-2 rounded-xl bg-[#2563EB] text-white shadow-sm font-black flex items-center gap-1.5">
            <Heart className="w-4 h-4 text-red-300 fill-red-300"/> Home
          </Link>

          <Link to="/donor/dashboard" className="px-4 py-2 rounded-xl text-slate-700 hover:bg-sky-50 flex items-center gap-1.5 transition-colors">
            <LayoutDashboard className="w-4 h-4 text-sky-600"/> Dashboard
          </Link>

          <Link to="/donor/emergency" className="px-4 py-2 rounded-xl text-slate-700 hover:bg-sky-50 flex items-center gap-1.5 transition-colors">
            <AlertTriangle className="w-4 h-4 text-amber-600"/> Emergency Requests
          </Link>

          <Link to="/donor/rare-blood" className="px-4 py-2 rounded-xl text-slate-700 hover:bg-sky-50 flex items-center gap-1.5 transition-colors">
            <ShieldCheck className="w-4 h-4 text-emerald-600"/> Rare Registry
          </Link>

          <Link to="/donor/profile" className="px-4 py-2 rounded-xl text-slate-700 hover:bg-sky-50 flex items-center gap-1.5 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {urgentRequests.map(req => (<div key={req.id} className="p-5 rounded-2xl bg-white border border-sky-100 shadow-xs space-y-3 hover:border-red-200 transition-all">
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between gap-2 text-xs">
                  ...
                  <button onClick={() => navigate('/donor/dashboard')} className="px-4 py-1.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs transition-colors">
                    Respond & Donate
                  </button>
                </div>
              </div>))}
