...
export const BloodBanksPage = () => {
    ...
    const filteredBanks = bloodBanks.filter(b => {
        if (searchCity !== 'ALL' && b.city.toLowerCase() !== searchCity.toLowerCase())
            return false;
        return true;
    });
    return (<div className="space-y-6 text-xs animate-in fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sky-100 pb-4">
        ...
        <select value={searchCity} onChange={e => setSearchCity(e.target.value)} className="p-2.5 rounded-xl bg-white border border-sky-200 text-slate-900 font-bold text-xs shadow-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredBanks.map((bank) => (<div key={bank.id} className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-8 gap-1.5 font-mono text-center">
              {bank.inventory.map(item => (<div key={item.group} className="p-2 rounded-xl bg-sky-50/50 border border-sky-100">
