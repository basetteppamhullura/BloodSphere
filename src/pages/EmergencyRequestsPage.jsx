...
import { AlertTriangle, PlusCircle, Check, X, Building2, MessageSquare } from 'lucide-react';
export const EmergencyRequestsPage = () => {
    const { requests, donors, donorRespondToRequest, setActiveEmergencyPostModal, openEmergencyChat } = useApp();
    const { currentUser } = useAuth();
    const [filterTab, setFilterTab] = useState('MATCHING_FOR_ME');
    ...
    const filteredRequests = requests.filter(r => {
        ...
        if (filterTab === 'CRITICAL')
            return r.urgency === 'CRITICAL' && r.status !== 'COMPLETED';
        if (filterTab === 'RARE')
            return ['O-', 'AB-', 'A-', 'Bombay Phenotype (O-h)'].includes(r.bloodGroup) && r.status !== 'COMPLETED';
        return true;
    });
    return (<div className="space-y-6 text-xs animate-in fade-in">
      ...
      {(!eligibility.isEligible || loggedInDonor.availabilityStatus !== 'AVAILABLE') && (<div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
      ...
      {/* Requests Grid */}
      {filteredRequests.length === 0 ? (<div className="p-8 rounded-3xl bg-white border border-sky-100 text-center text-slate-500">
          ...
          {filteredRequests.map(req => {
                ...
                return (<div key={req.id} className={`p-5 rounded-3xl bg-white border-2 transition-all space-y-4 shadow-sm ${req.urgency === 'CRITICAL' ? 'border-red-200 shadow-red-500/5' : 'border-sky-100'}`}>
                {/* CARD HEADER */}
                ...
                <div className="pt-1">
                  {myResponse || req.channelStatuses?.donorStatus === 'APPROVED' || req.channelStatuses?.donorStatus === 'REJECTED' ? (<div className="p-3 rounded-2xl bg-sky-50 border border-sky-100 text-slate-800 text-xs font-bold flex items-center justify-between">
                      ...
                      <button onClick={() => openEmergencyChat(req.id, loggedInDonor.id)} className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white font-extrabold text-[11px] flex items-center gap-1 shadow-xs transition-all hover:scale-105">
                        <MessageSquare className="w-3.5 h-3.5"/> 💬 Open Chat
                      </button>
                    </div>) : isSecured ? (<div className="p-3 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold text-center">
                      🔒 Required blood quantity has been secured for this patient.
                    </div>) : (<div className="grid grid-cols-2 gap-2">
                      <button onClick={() => donorRespondToRequest(req.id, loggedInDonor.id, 'ACCEPTED')} className="py-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 text-white font-extrabold text-xs shadow-md shadow-emerald-500/20 flex items-center justify-center gap-1.5 transition-all hover:scale-105 cursor-pointer">
                        <Check className="w-4 h-4"/> Approve / I Can Donate
                      </button>
                      <button onClick={() => donorRespondToRequest(req.id, loggedInDonor.id, 'DECLINED')} className="py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs border border-slate-200 flex items-center justify-center gap-1.5 transition-all cursor-pointer">
