...
/**
 * AI Lifesaver Chatbot Knowledge & Response Generator
 */
export function generateChatbotResponse(userQuery) {
  const query = userQuery.toLowerCase();

  if (query.includes("eligible") || query.includes("can i donate") || query.includes("weight") || query.includes("gap")) {
    return {
      reply: "To donate blood safely in India:\n1. Age: 18 – 65 years\n2. Minimum Weight: 45 kg\n3. Hemoglobin: At least 12.5 g/dL\n4. Frequency: 90 days gap between whole blood donations for males/females.\n5. You should be free from cold, fever, or ongoing antibiotics.\n\nUse our **AI Eligibility Predictor** tab on your profile for an instant diagnostic report!",
      suggestedActions: ["Test Eligibility", "View Emergency Requests"]
    };
  }

  if (query.includes("camp") || query.includes("drive") || query.includes("where")) {
    return {
      reply: "Upcoming Blood Donation Camps near you:\n📍 **Mega Independence Day Drive** @ KLE Tech Campus Hubballi (Aug 15)\n📍 **Community Lifesavers Camp** @ KCD Ground Dharwad (Aug 20)\n\nYou can RSVP directly on the **Donation Camps** tab to receive a free donor kit!",
      suggestedActions: ["View Camps", "RSVP Now"]
    };
  }

  if (query.includes("o-") || query.includes("rare") || query.includes("universal")) {
    return {
      reply: "O- Negative is the **Universal Red Cell Donor** blood group! Only ~5% of the population has O- blood, making it critical for emergency trauma surgeries when there is no time for cross-matching.\n\nIf you are O- negative, registering on our network sends high-priority SMS alerts to save lives!",
      suggestedActions: ["Register as O- Donor", "View Shortage Heatmap"]
    };
  }

  if (query.includes("request") || query.includes("need blood") || query.includes("urgent")) {
    return {
      reply: "To post an urgent blood request:\n1. Click **+ Emergency Request** at the top.\n2. You can type or speak in plain natural language (e.g. *'Need 2 units O- at KIMS Hospital Hubballi'*).\n3. Our AI will automatically parse the post and instantly notify nearby matched donors!",
      suggestedActions: ["Create Request", "Use AI Parser"]
    };
  }

  return {
    reply: "I am your AI Lifesaver Assistant! I can help you check donation eligibility, find nearby blood drives, post emergency blood requests, or understand blood group compatibility matrix. What would you like assistance with?",
