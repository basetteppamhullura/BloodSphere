...
import { Droplet, Search, AlertTriangle, Clock, CheckCircle2, Calendar, ShieldCheck, Eye, RefreshCw, X } from 'lucide-react';
...
export const HospitalBloodUnitDetails = () => {
    ...
    if (currentRole !== 'hospital' && currentRole !== 'admin') {
        return (<div className="max-w-2xl mx-auto my-12 p-8 rounded-3xl bg-white border border-red-200 text-center space-y-4 shadow-lg">
        ...
        <button onClick={() => navigate('/login/hospital')} className="px-5 py-2.5 rounded-2xl bg-slate-900 text-white font-extrabold text-xs">
          Hospital Login →
        </button>
      </div>);
    }
    const getStatusBadge = (status, daysRemaining) => {
        switch (status) {
            case 'Available':
                return (<span className="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 font-black text-[11px] inline-flex items-center gap-1">
            <span>🟢</span> Available
          </span>);
            case 'Reserved':
                return (<span className="px-2.5 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-300 font-black text-[11px] inline-flex items-center gap-1">
            <span>🟡</span> Reserved
          </span>);
            case 'Received':
                return (<span className="px-2.5 py-1 rounded-full bg-sky-100 text-sky-900 border border-sky-300 font-black text-[11px] inline-flex items-center gap-1">
            <span>🔵</span> Received
          </span>);
            case 'Near Expiry':
                return (<span className="px-2.5 py-1 rounded-full bg-orange-100 text-orange-900 border border-orange-300 font-black text-[11px] inline-flex items-center gap-1 animate-pulse">
            <span>🟠</span> Near Expiry ({daysRemaining}d left)
          </span>);
            case 'Expired':
                return (<span className="px-2.5 py-1 rounded-full bg-red-100 text-red-800 border border-red-300 font-black text-[11px] inline-flex items-center gap-1">
            <span>🔴</span> Expired
          </span>);
            case 'Issued':
                return (<span className="px-2.5 py-1 rounded-full bg-slate-200 text-slate-800 border border-slate-300 font-black text-[11px] inline-flex items-center gap-1">
            <span>⚫</span> Issued
          </span>);
            default:
                return (<span className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 font-bold text-[11px]">
            {status}
          </span>);
        }
    };
    return (<div className="max-w-6xl mx-auto space-y-6 pb-12 animate-in fade-in">
      ...
      {(nearExpiryUnits.length > 0 || expiredUnits.length > 0) && (<div className="p-5 rounded-3xl bg-gradient-to-r from-amber-50 via-orange-50 to-red-50 border border-amber-200/90 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setFilterStatus('Near Expiry')} className="px-3.5 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-black text-xs shrink-0 shadow-2xs">
              View Near Expiry →
            </button>
          </div>

          <div className="flex flex-wrap gap-2 text-xs">
            {nearExpiryUnits.slice(0, 3).map(u => (<span key={`warn-${u.unitId}`} className="px-3 py-1 rounded-xl bg-white border border-amber-300 text-amber-900 font-extrabold flex items-center gap-1.5 shadow-2xs">
      ...
      <div className="p-5 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-0">
            ...
            <input type="text" placeholder="Search by Unit ID (e.g. BU-2026-000125), storage location, or source..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-2xl bg-slate-50 border border-slate-200 text-xs font-bold text-slate-900 focus:outline-none focus:border-sky-500"/>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold">
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-2xl border border-slate-200">
              ...
              <select value={filterGroup} onChange={e => setFilterGroup(e.target.value)} className="bg-transparent text-slate-900 font-bold text-xs focus:outline-none">
                <option value="ALL">All Groups</option>
                {['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-', 'Bombay Phenotype (O-h)'].map(bg => (<option key={bg} value={bg}>{bg}</option>))}
              </select>
            </div>
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-2xl border border-slate-200">
              ...
              <select value={filterComponent} onChange={e => setFilterComponent(e.target.value)} className="bg-transparent text-slate-900 font-bold text-xs focus:outline-none">
            ...
            <div className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-2xl border border-slate-200">
              ...
              <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="bg-transparent text-slate-900 font-bold text-xs focus:outline-none">
            ...
            {(filterGroup !== 'ALL' || filterComponent !== 'ALL' || filterStatus !== 'ALL' || searchQuery !== '') && (<button onClick={() => {
                setFilterGroup('ALL');
      ...
      {filteredUnits.length === 0 ? (<div className="p-10 rounded-3xl bg-white border border-sky-100 text-center space-y-3 shadow-xs">
          ...
          {filteredUnits.map(unit => (<div key={unit.unitId} onClick={() => setSelectedUnit(unit)} className="p-5 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-4 hover:border-sky-400 hover:shadow-md transition-all cursor-pointer group">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-150 space-y-1">
                  ...
                  <span className={`text-xs font-black block flex items-center gap-1 ${unit.computedStatus === 'Expired' ? 'text-red-600' : unit.computedStatus === 'Near Expiry' ? 'text-orange-600' : 'text-slate-800'}`}>
      ...
      {selectedUnit && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-lg bg-white border border-sky-100 rounded-3xl p-6 space-y-5 text-xs shadow-2xl relative">
            
            <button onClick={() => setSelectedUnit(null)} className="absolute right-5 top-5 p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600">
              <X className="w-4 h-4"/>
            </button>
            ...
            <div className="pt-3 border-t border-sky-100 flex justify-end gap-2">
              <button onClick={() => setSelectedUnit(null)} className="px-5 py-2.5 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs">
                Close Inspector
              </button>
            </div>
          </div>
