...
export const HospitalEmergencyBoard: React.FC = () => {
  ...
  // Modal States
  const [viewingReq, setViewingReq] = useState<EmergencyRequest | null>(null);
  ...
  const filteredRequests = requests.filter((r) => {
    if (statusFilter !== 'ALL') {
      if (statusFilter === 'PENDING' && (r.status !== 'PENDING_HOSPITAL_APPROVAL' && r.status !== 'VERIFIED_SEARCHING_DONORS' && r.status !== 'PENDING')) return false;
      if (statusFilter === 'ACCEPTED' && (r.status !== 'APPROVED' && r.status !== 'BLOOD_SECURED')) return false;
      if (statusFilter === 'REJECTED' && r.status !== 'REJECTED') return false;
      if (statusFilter === 'REDIRECTED' && !r.trendingReason?.includes('Redirected')) return false;
      ...
      <div className="p-4 rounded-3xl bg-white border border-sky-100 shadow-sm flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap flex-1 min-w-[280px]">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search Request ID, patient, blood group, ward..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-3 py-2.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500"
            />
          </div>
          ...
          <select
            ...
            className="p-2.5 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 font-extrabold text-xs cursor-pointer"
          >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-sky-100 pb-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3.5 py-2.5 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs flex items-center gap-1 cursor-pointer transition-colors"
                    >
          ...
          <div className="w-full max-w-md bg-white border border-sky-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl relative">
            ...
            <form onSubmit={handleConfirmReject} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-red-600 text-white font-extrabold shadow-sm hover:bg-red-700 cursor-pointer"
                >
                  Confirm Rejection
                </button>
              </div>
            ...
            <form onSubmit={handleConfirmRedirect} className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500 cursor-pointer"
                >
                  {REGIONAL_TARGETS.map(t => (
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500"
                  required
                />
              </div>
