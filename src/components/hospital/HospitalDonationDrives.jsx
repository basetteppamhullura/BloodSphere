...
export const HospitalDonationDrives = () => {
    ...
    const [newTitle, setNewTitle] = useState('World Donor Day Special Drive');
    const [newDate, setNewDate] = useState('2026-08-28');
    const [newTime, setNewTime] = useState('09:00 AM - 02:00 PM');
    const [newLocation, setNewLocation] = useState('Vidyanagar Community Center');
    const [newTargetUnits, setNewTargetUnits] = useState(80);
    ...
    const handleRecordIntake = (driveId) => {
        setDrives(prev => prev.map(d => (d.id === driveId ? { ...d, unitsCollected: d.unitsCollected + 5 } : d)));
    ...
    return (<div className="space-y-6 text-xs animate-in fade-in w-full max-w-7xl mx-auto">
      ...
      <div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={() => setShowCreateModal(true)} className="px-4 py-2.5 rounded-2xl bg-red-600 hover:bg-red-500 text-white font-extrabold text-xs shadow-md flex items-center gap-1.5 shrink-0">
      ...
      <div className="space-y-4">
        {drives.map(drive => (<div key={drive.id} className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
            <div className="flex items-center justify-between border-b border-sky-100 pb-3">
              <div>
                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase ${drive.status === 'Active Today'
            ...
            <div className="space-y-1.5 text-slate-600 font-mono">
              ...
              <div className="flex items-center gap-1.5 pt-1 font-sans">
                ...
                {drive.targetGroups.map(grp => (<span key={grp} className="px-2.5 py-0.5 rounded-full bg-red-100 text-red-800 font-black text-[10px] border border-red-200">
                    {grp}
                  </span>))}
              </div>
            </div>
      ...
      {/* CREATE DRIVE MODAL */}
      {showCreateModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-sky-100 rounded-3xl p-6 space-y-4 shadow-2xl relative text-xs">
            ...
            <form onSubmit={handleCreateDrive} className="space-y-3">
              <div>
                ...
                <input type="text" value={newTitle} onChange={e => setNewTitle(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold" required/>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-slate-700 font-bold block mb-1">Date *</label>
                  <input type="date" value={newDate} onChange={e => setNewDate(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900" required/>
                </div>
                <div>
                  <label className="text-slate-700 font-bold block mb-1">Time *</label>
                  <input type="text" value={newTime} onChange={e => setNewTime(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900" required/>
                </div>
              </div>

              <div>
                <label className="text-slate-700 font-bold block mb-1">Target Units *</label>
                <input type="number" value={newTargetUnits} onChange={e => setNewTargetUnits(Number(e.target.value))} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold" required/>
              </div>
