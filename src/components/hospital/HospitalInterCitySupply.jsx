...
import { Truck, Building2, Navigation, Search, ShieldCheck, Send, MessageSquare, Eye, XCircle } from 'lucide-react';
export const HospitalInterCitySupply = () => {
    const { bloodBanks, interCityTransfers, createInterCityTransfer, updateInterCityTransferStatus, openEmergencyChat, showToast } = useApp();
    ...
    const [maxDistanceRadius, setMaxDistanceRadius] = useState(50);
    const [verifiedOnly, setVerifiedOnly] = useState(true);
    // Bank Online / Offline Status State Simulation Map
    const [bankConnectionMap, setBankConnectionMap] = useState({
        'bb_1': 'Online',
        'bb_2': 'Online',
        'bb_3': 'Online',
        'bb_4': 'Online'
    ...
    const [requestModalBank, setRequestModalBank] = useState(null);
    const [modalMode, setModalMode] = useState('request');
    const [requestGroup, setRequestGroup] = useState('O+');
    const [requestComponent, setRequestComponent] = useState('PRBC');
    ...
    const [hospitalName] = useState('KIMS Teaching Hospital & Trauma Center');
    const [notes, setNotes] = useState('Emergency trauma unit stock replenishment request');
    ...
    const handleOpenModal = (bank, mode) => {
        setRequestModalBank(bank);
        setModalMode(mode);
        setRequestGroup(selectedGroup !== 'ALL' ? selectedGroup : 'O+');
    };
    ...
    return (<div className="space-y-6 text-xs animate-in fade-in w-full max-w-7xl mx-auto pb-12">
      ...
      <div className="p-6 rounded-3xl bg-white border border-[#DDE8E2] shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <div>
            ...
            <input type="text" placeholder="e.g. Rotary, KIMS, Hubballi..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full p-2.5 rounded-xl bg-[#F7FAF8] border border-[#DDE8E2] text-slate-900 font-extrabold text-xs focus:outline-none focus:border-[#087443]"/>
          </div>

          <div>
            <label className="text-[10px] font-bold text-[#587067] uppercase block mb-1">Blood Type</label>
            <select value={selectedGroup} onChange={e => setSelectedGroup(e.target.value)} className="w-full p-2.5 rounded-xl bg-[#F7FAF8] border border-[#DDE8E2] text-slate-900 font-extrabold text-xs focus:outline-none focus:border-[#087443] cursor-pointer">
          ...
          <div>
            ...
            <select value={selectedComponent} onChange={e => setSelectedComponent(e.target.value)} className="w-full p-2.5 rounded-xl bg-[#F7FAF8] border border-[#DDE8E2] text-slate-900 font-extrabold text-xs focus:outline-none focus:border-[#087443] cursor-pointer">
          ...
          <div>
            ...
            <select value={maxDistanceRadius} onChange={e => setMaxDistanceRadius(Number(e.target.value))} className="w-full p-2.5 rounded-xl bg-[#F7FAF8] border border-[#DDE8E2] text-slate-900 font-extrabold text-xs focus:outline-none focus:border-[#087443] cursor-pointer">
          ...
          <div>
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full p-2.5 rounded-xl bg-[#F7FAF8] border border-[#DDE8E2] text-slate-900 font-extrabold text-xs focus:outline-none focus:border-[#087443] cursor-pointer">
      ...
      <div className="p-6 rounded-3xl bg-white border border-[#DDE8E2] space-y-4 shadow-sm">
        ...
        <div className="space-y-3">
          {interCityTransfers.length === 0 ? (<div className="p-6 text-center text-[#587067] font-bold bg-[#F7FAF8] rounded-2xl border border-[#DDE8E2]">
              No active inter-city transfer or supply requests.
            </div>) : (interCityTransfers.map(trf => (<div key={trf.id} className="p-4 rounded-2xl border border-[#DDE8E2] bg-[#F7FAF8] space-y-3 font-mono">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#DDE8E2] pb-2">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase font-sans flex items-center gap-1 ${trf.status === 'In Transit' || trf.status === 'Dispatched'
                ...
                {trf.status !== 'Delivered' && trf.status !== 'Received' && trf.status !== 'Completed' && (<div className="pt-1 flex items-center justify-end gap-2 font-sans">
                    {trf.status === 'Pending' && (<button onClick={() => updateInterCityTransferStatus(trf.id, 'In Transit')} className="px-3.5 py-1.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-extrabold text-[11px] shadow-xs">
                        Dispatch Courier (In Transit)
                      </button>)}
                    {(trf.status === 'In Transit' || trf.status === 'Dispatched') && (<button onClick={() => updateInterCityTransferStatus(trf.id, 'Delivered')} className="px-3.5 py-1.5 rounded-xl bg-[#087443] hover:bg-[#065b34] text-white font-extrabold text-[11px] shadow-xs">
      ...
      <div className="p-6 rounded-3xl bg-white border border-[#DDE8E2] space-y-4 shadow-sm">
        ...
        {filteredBanks.length === 0 ? (<div className="p-12 text-center text-[#587067] font-bold bg-[#F7FAF8] rounded-2xl border border-[#DDE8E2] space-y-2">
            ...
            {filteredBanks.map(bank => {
                ...
                const dist = bank.distanceKm || calculateDistanceKm(hospitalLocation.lat, hospitalLocation.lng, bank.lat, bank.lng);
                return (<div key={bank.id} className="p-6 rounded-3xl border border-[#DDE8E2] bg-white space-y-4 shadow-xs hover:shadow-md transition-all">
                  ...
                  <div className="flex items-start justify-between gap-3 border-b border-[#DDE8E2] pb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {bank.verified && (<span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-extrabold flex items-center gap-1 border border-emerald-300">
                    ...
                    <button onClick={() => toggleBankStatus(bank.id)} className={`px-2.5 py-1 rounded-full text-[10px] font-black border flex items-center gap-1 shrink-0 transition-all cursor-pointer ${connStatus === 'Online'
                        ? 'bg-[#E8F6EF] text-[#087443] border-[#DDE8E2]'
                        : connStatus === 'Limited'
                            ? 'bg-amber-100 text-amber-900 border-amber-300'
                            : 'bg-red-100 text-red-900 border-red-300'}`} title="Click to simulate live connection change">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-4 gap-1.5 text-center font-mono">
                      {bank.inventory.slice(0, 4).map((inv, iIdx) => (<div key={iIdx} className="p-2 rounded-xl bg-[#F7FAF8] border border-[#DDE8E2] space-y-0.5">
                  ...
                  {isExpanded && (<div className="p-4 rounded-2xl bg-[#E8F6EF]/60 border border-[#DDE8E2] space-y-3 animate-in fade-in">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse text-xs font-mono">
                          ...
                          <tbody className="divide-y divide-[#DDE8E2]">
                            {bank.inventory.map((inv, iIdx) => (<tr key={iIdx} className="hover:bg-white/60">
                  ...
                  <div className="pt-3 border-t border-[#DDE8E2] flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <button onClick={() => setExpandedBankId(isExpanded ? null : bank.id)} className="px-3 py-1.5 rounded-xl bg-[#F7FAF8] hover:bg-[#E8F6EF] text-[#18352A] font-extrabold text-xs border border-[#DDE8E2] transition-colors flex items-center gap-1">
                    ...
                    }} className="px-3 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-extrabold text-xs border border-indigo-200 transition-colors flex items-center gap-1" title="Contact Bank via Secure Hospital ↔ Blood Bank Chat">
                        <MessageSquare className="w-3.5 h-3.5"/> Contact Bank
                      </button>
                    </div>

                    <div className="flex items-center gap-2">
                      <button onClick={() => handleOpenModal(bank, 'request')} className="px-3.5 py-1.5 rounded-xl bg-[#087443] hover:bg-[#065b34] text-white font-extrabold text-xs shadow-xs transition-all flex items-center gap-1">
                        <Send className="w-3.5 h-3.5"/> Request Blood
                      </button>
                      <button onClick={() => handleOpenModal(bank, 'transfer')} className="px-3.5 py-1.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-extrabold text-xs shadow-xs transition-all flex items-center gap-1">
      ...
      {requestModalBank && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in">
            ...
            <form onSubmit={handleConfirmRequest} className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={requestGroup} onChange={e => setRequestGroup(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
                    <option value="O+">O+</option>
                    <option value="O-">O-</option>
                    <option value="A+">A+</option>
                    <option value="A-">A-</option>
                ...
                <div>
                  ...
                  <select value={requestComponent} onChange={e => setRequestComponent(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
                    <option value="PRBC">PRBC (Packed Red Blood Cells)</option>
                    <option value="Whole Blood">Whole Blood</option>
                    <option value="Plasma">Plasma (FFP)</option>
                    <option value="Platelets">Platelets (PRP)</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min={1} max={20} value={requestUnits} onChange={e => setRequestUnits(Number(e.target.value))} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold" required/>
                </div>

                <div>
                  <label className="text-slate-700 font-bold block mb-1">Urgency Level *</label>
                  <select value={requestUrgency} onChange={e => setRequestUrgency(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={patientId} onChange={e => setPatientId(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold font-mono"/>
                </div>

                <div>
                  <label className="text-slate-700 font-bold block mb-1">Required Date</label>
                  <input type="date" value={requiredDate} onChange={e => setRequiredDate(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold"/>
                </div>
              </div>

              <div>
                <label className="text-slate-700 font-bold block mb-1">Requesting Hospital</label>
                <input type="text" value={hospitalName} readOnly className="w-full p-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold"/>
              </div>

              <div>
                <label className="text-slate-700 font-bold block mb-1">Additional Notes</label>
                <textarea rows={2} value={notes} onChange={e => setNotes(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-medium"/>
              </div>

              <div className="pt-2 flex items-center justify-end gap-2">
                <button type="button" onClick={() => setRequestModalBank(null)} className="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-bold">
                  Cancel
                </button>
                <button type="submit" className="px-6 py-2.5 rounded-xl bg-[#087443] hover:bg-[#065b34] text-white font-extrabold shadow-md flex items-center gap-1.5">
