...
export const HospitalAuditLogViewer = ({ logs }) => {
    ...
    const filteredLogs = logs.filter(log => {
        const matchesSearch = (log.reason && log.reason.toLowerCase().includes(searchQuery.toLowerCase())) ||
    ...
    return (<div className="space-y-4 text-xs animate-in fade-in w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
        ...
        <button onClick={handleExportCsv} className="px-4 py-2 rounded-2xl bg-white hover:bg-sky-50 text-slate-700 font-bold text-xs border border-sky-200 flex items-center gap-1.5 shrink-0 shadow-xs">
      ...
      <div className="p-4 rounded-2xl bg-slate-50 border border-sky-100 space-y-3">
        <div className="relative">
          ...
          <input type="text" placeholder="Search by Reason text, Request ID, or Staff Name..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 text-xs focus:border-sky-600 focus:outline-none"/>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Filter Staff Member</label>
            <select value={filterStaff} onChange={e => setFilterStaff(e.target.value)} className="w-full p-2 rounded-xl bg-white border border-slate-200 text-slate-900 font-bold">
          ...
          <div>
            ...
            <select value={filterGroup} onChange={e => setFilterGroup(e.target.value)} className="w-full p-2 rounded-xl bg-white border border-slate-200 text-slate-900 font-bold">
          ...
          <div>
            ...
            <select value={filterChangeType} onChange={e => setFilterChangeType(e.target.value)} className="w-full p-2 rounded-xl bg-white border border-slate-200 text-slate-900 font-bold">
        ...
        <table className="w-full text-left border-collapse text-xs text-slate-700">
          ...
          <tbody className="divide-y divide-sky-100 font-mono text-[11px]">
            {filteredLogs.length === 0 ? (<tr>
                <td colSpan={9} className="p-6 text-center text-slate-500 font-sans">
              ...
              </tr>) : (filteredLogs.map(log => (<tr key={log.id} className="hover:bg-sky-50/40 transition-colors">
                  ...
                  <td className="p-3 font-sans">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase border ${log.changeType === 'Stock Added'
                ...
                : log.changeType === 'Stock Used'
                  ...
                  <td className="p-3 font-mono text-slate-500">
                    {log.linkedRequestId ? (<span className="px-2 py-0.5 rounded bg-sky-50 border border-sky-100 text-sky-700 font-bold">
                        {log.linkedRequestId}
                      </span>) : ('—')}
                  </td>
                </tr>)))}
