...
export const AccessDeniedBanner = () => {
    const { navigateTo, activePage } = useApp();
    const { currentRole } = useAuth();
    return (<div className="max-w-xl mx-auto my-12 p-8 rounded-3xl bg-slate-900 border-2 border-red-800/80 shadow-2xl text-center space-y-5 animate-in fade-in">
      ...
      <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
        <button onClick={() => navigateTo('dashboard')} className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-extrabold text-xs shadow-lg shadow-red-950 flex items-center justify-center gap-2">
