...
import { Navigation, AlertTriangle, Droplet, Clock, RefreshCw, Building2, ShieldCheck, Landmark, MapPin, Send } from 'lucide-react';
...
export const HospitalBloodStockFinder = () => {
    const { donors, inventoryStockMap, setActiveEmergencyPostModal, sendDirectRequestToDonor, showToast } = useApp();
    // 1. Search Form State
    ...
    const [locationQuery, setLocationQuery] = useState('Hubballi');
    const [stateQuery, setStateQuery] = useState('Karnataka');
    const [districtQuery, setDistrictQuery] = useState('Dharwad');
    const [pincodeQuery, setPincodeQuery] = useState('580021');
    ...
    const bloodGroupList = ['O+', 'O-', 'A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'Bombay Phenotype (O-h)'];
    const componentList = [
        'Whole Blood',
        'Plasma',
        'Platelets',
    ...
    const extractComponentStock = (stockObj) => {
        const grp = stockObj[selectedBloodGroup] || { whole: 0, plasma: 0, platelets: 0, rbc: 0 };
        if (selectedComponent === 'Red Blood Cells')
            return grp.rbc;
        if (selectedComponent === 'Whole Blood')
            return grp.whole;
        if (selectedComponent === 'Plasma')
            return grp.plasma;
        if (selectedComponent === 'Platelets')
            return grp.platelets;
        return grp.rbc;
    };
    // ----------------------------------------------------
    ...
    const processedDonors = donors
        ...
        .map((d) => {
        const distanceKm = calculateHaversineDistance(userCoords.lat, userCoords.lng, d.lat || 15.3647, d.lng || 75.1240);
    ...
    const handleRequestDonationFromDonor = (donor) => {
    ...
    const handleRequestBloodFromHospital = (hosp) => {
    ...
    const handleRequestBloodFromBloodBank = (bb) => {
        setActiveEmergencyPostModal(true);
        showToast(`Routing Direct Blood Request ID BR-1025 to ${bb.name} queue!`);
    };
    return (<div className="space-y-8 text-xs animate-in fade-in pb-12">
      {/* ================================================== */}
      ...
      <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {urgencyLevel === 'CRITICAL' && (<span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-red-100 text-red-800 border border-red-300 flex items-center gap-1 animate-bounce">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={handleUseMyLocation} disabled={isLocating} className={`px-4 py-3 rounded-2xl font-extrabold text-xs shadow-sm flex items-center gap-2 shrink-0 transition-all cursor-pointer ${isGPSActive
        ...
        {locationError && (<div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 flex items-start gap-2.5 animate-in fade-in">
        ...
        <form onSubmit={handleExecuteSearch} className="space-y-5">
          ...
          <div>
            ...
            return (<button key={bg} type="button" onClick={() => setSelectedBloodGroup(bg)} className={`px-3.5 py-2 rounded-2xl font-extrabold text-xs transition-all cursor-pointer ${isSelected
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
            ...
            <div>
              ...
              <select value={selectedComponent} onChange={(e) => setSelectedComponent(e.target.value)} className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-red-500 cursor-pointer">
                {componentList.map((c) => (<option key={c} value={c}>
                    {c}
                  </option>))}
              </select>
            </div>
            ...
            <div>
              ...
              <input type="number" min={1} max={20} value={unitsRequired} onChange={(e) => setUnitsRequired(Math.max(1, parseInt(e.target.value) || 1))} className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-red-500" required/>
            </div>

            {/* SEARCH RADIUS */}
            <div>
              <label className="text-slate-700 font-extrabold block mb-1">Search Radius *</label>
              <select value={searchRadiusKm} onChange={(e) => setSearchRadiusKm(parseInt(e.target.value))} className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-red-500 cursor-pointer">
            ...
            <div>
              ...
              <div className="relative">
                <input type="text" value={locationQuery} onChange={(e) => {
            setLocationQuery(e.target.value);
            setIsGPSActive(false);
        }} placeholder="e.g. Hubballi, Dharwad" className="w-full p-3 pl-8 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-red-500"/>
            ...
            <div>
              ...
              <select value={urgencyLevel} onChange={(e) => setUrgencyLevel(e.target.value)} className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-red-500 cursor-pointer">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1 border-t border-slate-100">
            <div>
              ...
              <input type="text" value={`${districtQuery}, ${stateQuery}`} onChange={(e) => setDistrictQuery(e.target.value)} placeholder="Dharwad, Karnataka" className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 font-medium text-xs"/>
            </div>

            <div>
              <label className="text-slate-500 font-bold block mb-1">Required By Date (Optional)</label>
              <input type="date" value={requiredDate} onChange={(e) => setRequiredDate(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 font-medium text-xs"/>
            </div>

            <div>
              <label className="text-slate-500 font-bold block mb-1">Pincode (Optional)</label>
              <input type="text" value={pincodeQuery} onChange={(e) => setPincodeQuery(e.target.value)} placeholder="580021" className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 font-medium text-xs"/>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
            ...
            <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-2xl">
              <button type="button" onClick={() => setActiveCategoryFilter('ALL')} className={`px-3 py-1.5 rounded-xl font-extrabold text-[11px] transition-all cursor-pointer ${activeCategoryFilter === 'ALL'
            ? 'bg-white text-slate-900 shadow-xs'
            : 'text-slate-600 hover:text-slate-900'}`}>
                All 3 Sources
              </button>
              <button type="button" onClick={() => setActiveCategoryFilter('DONORS')} className={`px-3 py-1.5 rounded-xl font-extrabold text-[11px] transition-all cursor-pointer ${activeCategoryFilter === 'DONORS'
            ? 'bg-red-600 text-white shadow-xs'
            : 'text-slate-600 hover:text-slate-900'}`}>
                🩸 Donors ({processedDonors.length})
              </button>
              <button type="button" onClick={() => setActiveCategoryFilter('HOSPITALS')} className={`px-3 py-1.5 rounded-xl font-extrabold text-[11px] transition-all cursor-pointer ${activeCategoryFilter === 'HOSPITALS'
            ? 'bg-sky-600 text-white shadow-xs'
            : 'text-slate-600 hover:text-slate-900'}`}>
                🏥 Hospitals ({processedHospitals.length})
              </button>
              <button type="button" onClick={() => setActiveCategoryFilter('BLOOD_BANKS')} className={`px-3 py-1.5 rounded-xl font-extrabold text-[11px] transition-all cursor-pointer ${activeCategoryFilter === 'BLOOD_BANKS'
            ...
            <div className="flex items-center gap-2">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="p-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800 font-extrabold text-xs focus:outline-none cursor-pointer">
              ...
              <button type="submit" disabled={isSearching} className="px-6 py-3.5 rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white font-black text-xs shadow-lg shadow-red-500/20 flex items-center gap-2 cursor-pointer transition-all hover:scale-102">
      ...
      {/* ------------------------------------------------------------------------- */}
      {(activeCategoryFilter === 'ALL' || activeCategoryFilter === 'DONORS') && (<div className="space-y-4">
          {/* SECTION HEADER */}
          ...
          {isSearching ? (<div className="p-8 rounded-3xl bg-white border border-slate-200 text-center space-y-2">
            ...
            </div>) : processedDonors.length === 0 ? (<div className="p-8 rounded-3xl bg-white border border-slate-200 text-center space-y-3">
              ...
              {processedDonors.map((donor) => {
                    ...
                    return (<div key={donor.id} className={`p-5 rounded-3xl bg-white border transition-all space-y-4 relative overflow-hidden flex flex-col justify-between ${isAvailable
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${donor.statusLabel === 'AVAILABLE'
                    ...
                    <div className="space-y-2 pt-1 border-t border-slate-100">
                      ...
                      <button type="button" onClick={() => handleRequestDonationFromDonor(donor)} className="w-full py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-black text-xs shadow-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer hover:scale-101">
      ...
      {/* ------------------------------------------------------------------------- */}
      {(activeCategoryFilter === 'ALL' || activeCategoryFilter === 'HOSPITALS') && (<div className="space-y-4">
          {/* SECTION HEADER */}
          ...
          {isSearching ? (<div className="p-8 rounded-3xl bg-white border border-slate-200 text-center space-y-2">
            ...
            </div>) : processedHospitals.length === 0 ? (<div className="p-8 rounded-3xl bg-white border border-slate-200 text-center space-y-3">
              ...
              <button type="button" onClick={() => setSearchRadiusKm(100)} className="px-4 py-2 rounded-xl bg-sky-50 text-sky-700 border border-sky-200 font-extrabold text-xs hover:bg-sky-100 transition-colors cursor-pointer">
                Expand Search Radius to 100 km
              </button>
              ...
              {processedHospitals.map((hosp) => {
                    ...
                    return (<div key={hosp.id} className={`p-6 rounded-3xl bg-white border transition-all space-y-4 relative overflow-hidden flex flex-col justify-between ${isAvailable
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {hosp.verified && (<span className="text-sky-600" title="State Verified Hospital">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 ${hosp.status === 'AVAILABLE'
                            ? 'bg-emerald-100 text-emerald-800'
                            : hosp.status === 'LIMITED'
                                ? 'bg-amber-100 text-amber-800'
                    ...
                    <div className="grid grid-cols-4 gap-2 py-3 px-4 rounded-2xl bg-slate-50 border border-slate-100 text-center">
                      ...
                      <div>
                        ...
                        <strong className={`text-sm font-black ${hosp.availableUnits > 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                    ...
                    <div className="flex items-center justify-between gap-3 pt-2 border-t border-slate-100">
                      ...
                      <button type="button" onClick={() => handleRequestBloodFromHospital(hosp)} disabled={hosp.availableUnits === 0} className={`px-5 py-2.5 rounded-xl font-black text-xs shadow-sm flex items-center gap-1.5 transition-all cursor-pointer ${hosp.availableUnits > 0
      ...
      {/* ------------------------------------------------------------------------- */}
      {(activeCategoryFilter === 'ALL' || activeCategoryFilter === 'BLOOD_BANKS') && (<div className="space-y-4">
          {/* SECTION HEADER */}
          ...
          {isSearching ? (<div className="p-8 rounded-3xl bg-white border border-slate-200 text-center space-y-2">
            ...
            </div>) : processedBloodBanks.length === 0 ? (<div className="p-8 rounded-3xl bg-white border border-slate-200 text-center space-y-3">
              ...
              <button type="button" onClick={() => setSearchRadiusKm(100)} className="px-4 py-2 rounded-xl bg-emerald-50 text-emerald-700 border border-emerald-200 font-extrabold text-xs hover:bg-emerald-100 transition-colors cursor-pointer">
                Expand Search Radius to 100 km
              </button>
            </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {processedBloodBanks.map((bb) => {
                    const isAvailable = bb.status === 'AVAILABLE';
                    return (<div key={bb.id} className={`p-6 rounded-3xl bg-white border transition-all space-y-4 relative overflow-hidden flex flex-col justify-between ${isAvailable
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 ${bb.status === 'AVAILABLE'
                            ? 'bg-emerald-100 text-emerald-800'
                            : bb.status === 'LIMITED'
                                ? 'bg-amber-100 text-amber-800'
                                : 'bg-red-100 text-red-800'}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 py-3 px-3 rounded-2xl bg-slate-50 border border-slate-100 text-center">
                      ...
                      <div>
                        ...
                        <strong className={`text-sm font-black ${bb.availableUnits > 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                    ...
                    <div className="space-y-2 pt-1 border-t border-slate-100">
                      ...
                      <button type="button" onClick={() => handleRequestBloodFromBloodBank(bb)} disabled={bb.availableUnits === 0} className={`w-full py-2.5 rounded-xl font-black text-xs shadow-sm flex items-center justify-center gap-1.5 transition-all cursor-pointer ${bb.availableUnits > 0
