...
import { ShieldAlert, Search, Building2, MapPin, Send, Droplet, Navigation, PlusCircle } from 'lucide-react';
const REGIONAL_FACILITIES = [
    {
        id: 'fac_kims',
...
export const RequesterActionHub = () => {
    const { createEmergencyRequest, setActiveEmergencyPostModal, showToast } = useApp();
    // Requester GPS Location State (Default: Hubballi)
    const [requesterLoc, setRequesterLoc] = useState({
        lat: 15.3647,
        lng: 75.124
    });
    // Filter Selection State
    const [selectedGroup, setSelectedGroup] = useState('O-');
    const [selectedRadius, setSelectedRadius] = useState(1000); // Default Any distance
    // Applied Active Filter State (Updated on Search click)
    const [appliedGroup, setAppliedGroup] = useState('O-');
    const [appliedRadius, setAppliedRadius] = useState(1000);
    // Emergency Express Form Modal State
    const [showExpressModal, setShowExpressModal] = useState(false);
    const [expPatientName, setExpPatientName] = useState('Ramesh Kumar');
    const [expGroup, setExpGroup] = useState('O-');
    const [expUnits, setExpUnits] = useState(2);
    const [expHospital, setExpHospital] = useState('KIMS Teaching Hospital');
    const [expPhone, setExpPhone] = useState('+91 98450 12345');
    ...
    const handleEmergencyExpressSubmit = (e) => {
        e.preventDefault();
    ...
    return (<div className="space-y-6 text-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-3xl bg-gradient-to-br from-red-950 via-slate-900 to-slate-900 border-2 border-red-600 shadow-2xl space-y-4 flex flex-col justify-between hover:scale-[1.01] transition-transform">
          ...
          <button onClick={() => setShowExpressModal(true)} className="w-full py-3 rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white font-black text-sm shadow-xl shadow-red-950/80 flex items-center justify-center gap-2 transition-all hover:scale-105">
        ...
        <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl space-y-4 flex flex-col justify-between hover:border-slate-700 transition-all">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleUseCurrentGPS} className="text-[10px] font-bold text-emerald-400 hover:underline flex items-center gap-1">
          ...
          <button onClick={() => setActiveEmergencyPostModal(true)} className="w-full py-3 rounded-2xl bg-slate-800 hover:bg-slate-700 text-white font-extrabold text-xs border border-slate-700 shadow-md flex items-center justify-center gap-2">
      ...
      <div className="p-6 rounded-3xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800">
              ...
              <select value={selectedGroup} onChange={e => setSelectedGroup(e.target.value)} className="bg-transparent text-white font-black focus:outline-none">
                {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', 'Bombay Phenotype (O-h)'].map(bg => (<option key={bg} value={bg} className="bg-slate-900 text-white">{bg}</option>))}
              </select>
            </div>
            ...
            <div className="flex items-center gap-1.5 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800">
              ...
              <select value={selectedRadius} onChange={e => setSelectedRadius(Number(e.target.value))} className="bg-transparent text-white font-bold focus:outline-none">
            ...
            <button onClick={handleExecuteSearch} className="px-4 py-1.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white font-black text-xs shadow-md flex items-center gap-1.5 transition-all hover:scale-105">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {facilitiesWithDistance.map(fac => {
            ...
            return (<div key={fac.id} className="p-5 rounded-3xl bg-slate-950 border border-slate-800 space-y-3 flex flex-col justify-between shadow-lg">
                <div>
                  <div className="flex items-center justify-between">
      ...
      {showExpressModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in">
          <div className="w-full max-w-md bg-slate-900 border-2 border-red-600 rounded-3xl p-6 space-y-4 text-xs shadow-2xl">
            ...
            <form onSubmit={handleEmergencyExpressSubmit} className="space-y-3">
              <div>
                ...
                <input type="text" value={expPatientName} onChange={e => setExpPatientName(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-bold" required/>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-slate-300 font-bold block mb-1">Blood Group *</label>
                  <select value={expGroup} onChange={e => setExpGroup(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-bold">
                ...
                <div>
                  ...
                  <input type="number" min={1} max={10} value={expUnits} onChange={e => setExpUnits(Number(e.target.value))} className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-bold" required/>
                </div>
              </div>

              <div>
                <label className="text-slate-300 font-bold block mb-1">Hospital / Location *</label>
                <input type="text" value={expHospital} onChange={e => setExpHospital(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-bold" required/>
              </div>

              <div>
                <label className="text-slate-300 font-bold block mb-1">Contact Phone *</label>
                <input type="text" value={expPhone} onChange={e => setExpPhone(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white font-bold" required/>
              </div>
