...
export const BloodBankPortalDesk = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const { requests, inventoryStockMap, bloodUnitsList, activityLogs, notifications, reserveBloodBankUnits, issueBloodBankUnits, acceptBloodRequest, rejectBloodBankRequest, openEmergencyChat, intakeBloodUnit, checkBloodUnitExpiries, showToast, createInterCityTransfer } = useApp();
    ...
    const getActiveTabFromPath = () => {
        ...
        if (path.includes('/bloodbank/hospital-requests'))
            return 'hospital-queue';
        if (path.includes('/bloodbank/bloodbank-requests'))
            return 'bloodbank-queue';
        if (path.includes('/bloodbank/requests') || path.includes('/bloodbank/reservations'))
            return 'direct-queue';
        if (path.includes('/bloodbank/inventory'))
            return 'inventory';
        if (path.includes('/bloodbank/units'))
            return 'lifecycle';
        if (path.includes('/bloodbank/preservation'))
            return 'preservation';
        if (path.includes('/bloodbank/issue'))
            return 'issue';
        if (path.includes('/bloodbank/alerts'))
            return 'alerts';
        if (path.includes('/bloodbank/notifications'))
            return 'notifications';
        if (path.includes('/bloodbank/activity'))
            return 'activity';
        if (path.includes('/bloodbank/reports'))
            return 'reports';
        if (path.includes('/bloodbank/settings'))
            return 'settings';
        return 'dashboard';
    };
    ...
    const [rejectReason, setRejectReason] = useState('Blood unavailable in storage');
    const [customRejectReason, setCustomRejectReason] = useState('');
    const [reserveModalReq, setReserveModalReq] = useState(null);
    const [selectedUnitsToReserve, setSelectedUnitsToReserve] = useState([]);
    const [issueModalReq, setIssueModalReq] = useState(null);
    const [selectedUnitIdToIssue, setSelectedUnitIdToIssue] = useState('');
    const [receivingPartyInput, setReceivingPartyInput] = useState('');
    const [showIntakeModal, setShowIntakeModal] = useState(false);
    const [intakeGroup, setIntakeGroup] = useState('O+');
    const [intakeComp, setIntakeComp] = useState('PRBC');
    const [intakeStorageLoc, setIntakeStorageLoc] = useState('Main Vault R-01 (Shelf A)');
    const [intakeDonorRef, setIntakeDonorRef] = useState('Voluntary Regular Donor');
    ...
    const directRequestsList = useMemo(() => {
        return activeRequestsQueue.filter(r => {
            if (r.requesterType === 'DIRECT_REQUESTER')
                return true;
            if (r.requesterType === 'HOSPITAL' || r.requesterType === 'BLOOD_BANK')
                return false;
            const isHospital = r.hospitalName && (r.hospitalName.toLowerCase().includes('hospital') ||
            ...
            const isBank = r.hospitalName && (r.hospitalName.toLowerCase().includes('blood bank') ||
                r.hospitalName.toLowerCase().includes('blood center') ||
                r.hospitalName.toLowerCase().includes('regional blood'));
            if (isBank)
                return false;
            if (isHospital && r.isVerifiedByHospital)
                return false;
            return true;
        }).sort((a, b) => {
    ...
    const hospitalRequestsList = useMemo(() => {
        return activeRequestsQueue.filter(r => {
            if (r.requesterType === 'HOSPITAL')
                return true;
            if (r.requesterType === 'DIRECT_REQUESTER' || r.requesterType === 'BLOOD_BANK')
                return false;
            const isHospital = r.hospitalName && (r.hospitalName.toLowerCase().includes('hospital') ||
                r.hospitalName.toLowerCase().includes('kims') ||
                r.hospitalName.toLowerCase().includes('sdm') ||
    ...
    const bloodBankRequestsList = useMemo(() => {
        return activeRequestsQueue.filter(r => {
            if (r.requesterType === 'BLOOD_BANK')
                return true;
            const isBank = r.hospitalName && (r.hospitalName.toLowerCase().includes('blood bank') ||
                r.hospitalName.toLowerCase().includes('blood center') ||
                r.hospitalName.toLowerCase().includes('rotary') ||
                r.hospitalName.toLowerCase().includes('regional blood'));
            const isTransfer = r.reason && (r.reason.toLowerCase().includes('transfer') ||
    ...
    const lowStockAlertGroups = useMemo(() => {
        ...
        return allGroups.filter(group => {
            ...
            const avail = Number(Object.values(comps).reduce((acc, item) => acc + Number(item?.available || 0), 0));
    ...
    const getAvailableStockCount = (bg, comp) => {
        const groupStock = inventoryStockMap[bg] || {};
        if (comp && groupStock[comp]) {
            return groupStock[comp]?.available || 0;
        }
        return Number(Object.values(groupStock).reduce((acc, item) => acc + Number(item?.available || 0), 0));
    };
    // ==================================================
    // REQUEST ACTIONS
    // ==================================================
    ...
    const handleAcceptRequest = (req) => {
        const comp = req.bloodComponent || 'PRBC';
        const avail = getAvailableStockCount(req.bloodGroup, comp);
        if (avail < req.unitsNeeded) {
            const proceed = window.confirm(`⚠️ Stock Notice: Currently only ${avail} unit(s) of ${req.bloodGroup} (${comp}) available in the vault (Requested: ${req.unitsNeeded}u). Do you still want to accept and coordinate fulfillment?`);
            if (!proceed)
                return;
        }
    ...
    const handleOpenReserveModal = (req) => {
        setReserveModalReq(req);
        // Find eligible stored/available units matching group & component
        const comp = req.bloodComponent || 'PRBC';
        const eligible = bloodUnitsList.filter(u => u.bloodGroup === req.bloodGroup && (u.component === comp || !u.component) && (u.status === 'STORED' || u.status === 'APPROVED' || u.status === 'COLLECTED'));
    ...
    const handleConfirmReserve = () => {
        if (!reserveModalReq)
            return;
        const comp = reserveModalReq.bloodComponent || 'PRBC';
    ...
    // 6. Open Issue Modal
    const handleOpenIssueModal = (req) => {
        setIssueModalReq(req);
        setReceivingPartyInput(req.hospitalName || req.contactPerson || 'Authorized Recipient');
        const matchingUnit = bloodUnitsList.find(u => u.bloodGroup === req.bloodGroup && (u.status === 'RESERVED' || u.status === 'STORED' || u.status === 'APPROVED'));
    ...
    const handleReplenishmentOrder = (targetInstName, group) => {
    ...
    const renderRequestCard = (req, queueType) => {
        ...
        return (<div key={req.id} className={`p-4 sm:p-5 rounded-2xl border transition-all ${isCritical
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-2 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${isCritical ? 'bg-red-600 text-white animate-pulse' : 'bg-amber-100 text-amber-900 border border-amber-300'}`}>
                {req.urgency}
              </span>
              ...
              {(() => {
                ...
                return (<span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase flex items-center gap-1 border ${bankStatus === 'APPROVED'
            ...
            <div className="text-xs text-slate-600 space-y-1">
              ...
              {req.reason && (<p className="text-[11px] text-slate-600 italic bg-white/70 p-2 rounded-xl border border-slate-100">
                  Reason: "{req.reason}"
                </p>)}
            </div>
            ...
            <div className="pt-1 flex items-center gap-2 text-xs">
              ...
              <span className={`px-2 py-0.5 rounded-md font-mono font-bold text-[11px] ${hasEnoughStock ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800'}`}>
          ...
          <div className="flex flex-wrap md:flex-col items-stretch gap-2 shrink-0 justify-end">
            <button onClick={() => setSelectedRequestForDetails(req)} className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer">
              <Eye className="w-3.5 h-3.5"/> View Details
            </button>

            {req.status !== 'APPROVED' && req.status !== 'BLOOD_SECURED' && (<button onClick={() => handleAcceptRequest(req)} className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs shadow-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer">
                <Check className="w-4 h-4"/> Accept Request
              </button>)}

            <button onClick={() => handleOpenReserveModal(req)} className="px-3.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-black text-xs shadow-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer">
              <Lock className="w-3.5 h-3.5"/> Reserve Units
            </button>

            <button onClick={() => handleOpenIssueModal(req)} className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-500 text-white font-black text-xs shadow-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer">
              <Send className="w-3.5 h-3.5"/> Issue Blood
            </button>

            <button onClick={() => handleOpenRejectModal(req)} className="px-3.5 py-1.5 rounded-xl bg-slate-50 hover:bg-red-50 text-slate-500 hover:text-red-700 font-bold text-xs border border-slate-200 flex items-center justify-center gap-1 cursor-pointer">
              <XCircle className="w-3.5 h-3.5"/> Reject
            </button>

            <button onClick={() => openEmergencyChat(req.id)} className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 text-white font-extrabold text-xs shadow-xs flex items-center justify-center gap-1.5 cursor-pointer">
    ...
    return (<div className="max-w-6xl mx-auto space-y-6 pb-20 animate-in fade-in">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-emerald-800 via-teal-900 to-slate-900 text-white shadow-xl relative overflow-hidden border border-emerald-700/40">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={() => setShowIntakeModal(true)} className="px-4 py-2.5 rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-white font-extrabold text-xs shadow-md flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer">
              <PlusCircle className="w-4 h-4"/> Intake New Stock
            </button>
            <button onClick={checkBloodUnitExpiries} className="px-3.5 py-2.5 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs border border-white/20 flex items-center gap-1.5 transition-all cursor-pointer" title="Scan for expired units">
              <RefreshCw className="w-4 h-4 text-emerald-300"/> Scan Expiries
            </button>
            <button onClick={handleSetBankGPS} className="px-3.5 py-2.5 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs border border-white/20 flex items-center gap-1.5 transition-all cursor-pointer" title="Update GPS Location">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2.5 text-xs">
                ...
                return (<div key={group} className={`p-3 rounded-2xl border text-center space-y-1 ${isLow ? 'bg-red-50/60 border-red-200' : 'bg-slate-50/70 border-slate-200'}`}>
            ...
            <div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-4">
              ...
              <div className="space-y-2.5 text-xs">
                {nearbyWithDistance.slice(0, 4).map(inst => (<div key={inst.id} className="p-3 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                    <div>
                    ...
                    {inst.lowStockGroup ? (<button onClick={() => handleReplenishmentOrder(inst.name, inst.lowStockGroup)} className="px-3 py-1.5 rounded-xl bg-amber-100 text-amber-900 hover:bg-amber-200 border border-amber-300 font-extrabold text-[11px] cursor-pointer">
                        Supply {inst.lowStockGroup}
                      </button>) : (<button onClick={() => handleReplenishmentOrder(inst.name, 'O+')} className="px-3 py-1.5 rounded-xl bg-sky-100 text-sky-900 hover:bg-sky-200 border border-sky-300 font-extrabold text-[11px] cursor-pointer">
                        Request Transfer
                      </button>)}
                  </div>))}
              </div>
      ...
      {activeTab === 'direct-queue' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          ...
          {directRequestsList.length === 0 ? (<div className="p-10 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-2">
      ...
      {activeTab === 'hospital-queue' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-4">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          {hospitalRequestsList.length === 0 ? (<div className="p-10 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-2">
      ...
      {activeTab === 'bloodbank-queue' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-4">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          {bloodBankRequestsList.length === 0 ? (<div className="p-10 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-2">
      ...
      {activeTab === 'inventory' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-4">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setShowIntakeModal(true)} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs shadow-sm flex items-center gap-1.5 cursor-pointer">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-', 'Bombay Phenotype (O-h)'].map(group => {
                ...
                const totalReserved = Number(Object.values(groupStock).reduce((acc, c) => acc + Number(c?.reserved || 0), 0));
                const totalIssued = Number(Object.values(groupStock).reduce((acc, c) => acc + Number(c?.issued || 0), 0));
                const isLow = totalGroupAvail < minStockThreshold;
                return (<tr key={group} className="hover:bg-sky-50/40 transition-colors">
                      ...
                      <td className="py-3.5 px-4 font-sans">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase inline-flex items-center gap-1 ${isLow ? 'bg-red-100 text-red-800 border border-red-300' : 'bg-emerald-100 text-emerald-800 border border-emerald-300'}`}>
      ...
      {activeTab === 'lifecycle' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-4">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <button onClick={() => setShowIntakeModal(true)} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs shadow-sm flex items-center gap-1.5 cursor-pointer shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" placeholder="Search Barcode Unit ID (e.g. BU-10025)..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-8 pr-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs font-bold"/>
            </div>

            <select value={filterGroup} onChange={e => setFilterGroup(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs">
              <option value="ALL">All Groups</option>
              {['A+', 'A-', 'B+', 'B-', 'O+', 'O-', 'AB+', 'AB-'].map(bg => (<option key={bg} value={bg}>{bg}</option>))}
            </select>

            <select value={filterComponent} onChange={e => setFilterComponent(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs">
            ...
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {filteredBloodUnits.map(unit => (<tr key={unit.unitId} className="hover:bg-sky-50/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase inline-flex items-center gap-1 ${unit.status === 'STORED' || unit.status === 'APPROVED' || unit.status === 'COLLECTED'
      ...
      {activeTab === 'preservation' && (<div className="space-y-6">
          ...
          <div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
            <div className="flex items-center justify-between border-b border-sky-100 pb-4">
              ...
              <button onClick={checkBloodUnitExpiries} className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs flex items-center gap-1.5 cursor-pointer">
                <RefreshCw className="w-3.5 h-3.5"/> Scan Expiries
              </button>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {vaultTemps.map(v => (<div key={v.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-3xl bg-white border border-amber-200 shadow-xs space-y-4">
              ...
              {nearExpiryUnitsList.length === 0 ? (<p className="text-xs text-slate-500 italic">No units approaching expiration within the next 5 days.</p>) : (<div className="space-y-2 max-h-64 overflow-y-auto">
                  {nearExpiryUnitsList.map(u => (<div key={u.unitId} className="p-3 rounded-xl bg-amber-50/60 border border-amber-200 flex items-center justify-between text-xs">
                      <div>
            ...
            <div className="p-6 rounded-3xl bg-white border border-red-200 shadow-xs space-y-4">
              ...
              {expiredUnitsList.length === 0 ? (<p className="text-xs text-slate-500 italic">No expired blood units found. Inventory is clean.</p>) : (<div className="space-y-2 max-h-64 overflow-y-auto">
                  {expiredUnitsList.map(u => (<div key={u.unitId} className="p-3 rounded-xl bg-red-50/60 border border-red-200 flex items-center justify-between text-xs">
                      <div>
                        <strong className="font-black text-slate-900">{u.unitId}</strong> — {u.bloodGroup} ({u.component})
      ...
      {activeTab === 'issue' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          ...
          {activeRequestsQueue.length === 0 ? (<div className="p-10 rounded-2xl bg-slate-50 border border-slate-200 text-center space-y-2">
              <CheckCircle2 className="w-8 h-8 text-emerald-600 mx-auto"/>
              ...
              {activeRequestsQueue.map(req => (<div key={req.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => handleOpenIssueModal(req)} className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer">
      ...
      {activeTab === 'alerts' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          <div className="flex items-center justify-between border-b border-sky-100 pb-4">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="number" min={1} max={20} value={minStockThreshold} onChange={e => setMinStockThreshold(Number(e.target.value))} className="w-16 p-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-bold text-center"/>
            </div>
          </div>

          {lowStockAlertGroups.length === 0 ? (<div className="p-8 rounded-2xl bg-emerald-50 border border-emerald-200 text-center space-y-2">
              <CheckCircle2 className="w-8 h-8 text-emerald-600 mx-auto"/>
              ...
              {lowStockAlertGroups.map(group => {
                    const comps = inventoryStockMap[group] || {};
                    const currentAvail = Number(Object.values(comps).reduce((acc, item) => acc + Number(item?.available || 0), 0));
                    return (<div key={group} className="p-4 rounded-2xl bg-red-50/70 border border-red-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <button onClick={() => handleReplenishmentOrder('Rotary Regional Blood Bank Dharwad', group)} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white font-extrabold text-xs shadow-xs shrink-0 cursor-pointer">
      ...
      {activeTab === 'activity' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-5">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {activityLogs.map(log => (<tr key={log.activityId} className="hover:bg-sky-50/40 transition-colors">
      ...
      {activeTab === 'reports' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-4">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              {['today', '7days', '30days'].map(range => (<button key={range} onClick={() => setReportRange(range)} className={`px-3 py-1.5 rounded-xl text-xs font-extrabold capitalize cursor-pointer ${reportRange === range ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-700'}`}>
      ...
      {/* TAB 13: SETTINGS */}
      {activeTab === 'settings' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 shadow-xs space-y-6">
          <div className="border-b border-sky-100 pb-4">
            <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
      ...
      {selectedRequestForDetails && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-lg bg-white border border-sky-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl">
            <div className="flex items-center justify-between border-b border-sky-100 pb-3">
              ...
              <button onClick={() => setSelectedRequestForDetails(null)} className="text-slate-400 hover:text-slate-700 p-1">
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2 text-slate-700">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                ...
                {selectedRequestForDetails.doctorName && (<p><strong>Doctor In-Charge:</strong> Dr. {selectedRequestForDetails.doctorName}</p>)}
                {selectedRequestForDetails.reason && (<p className="italic text-slate-600"><strong>Reason:</strong> "{selectedRequestForDetails.reason}"</p>)}
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setSelectedRequestForDetails(null)} className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold">
                Close
              </button>
            </div>
          </div>
      ...
      {rejectModalReq && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-red-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
              ...
              {rejectReason === 'Other' && (<div>
                  <label className="text-slate-700 font-bold block mb-1">Specify Reason *</label>
                  <input type="text" placeholder="Enter rejection reason..." value={customRejectReason} onChange={e => setCustomRejectReason(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold"/>
                </div>)}
            </div>

            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setRejectModalReq(null)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold">
                Cancel
              </button>
              <button onClick={handleConfirmReject} className="px-5 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white font-black shadow-md shadow-red-600/20 cursor-pointer">
                Confirm Rejection
              </button>
            </div>
          </div>
      ...
      {reserveModalReq && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-amber-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl">
            ...
            <div>
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto p-2 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                .map(unit => (<label key={unit.unitId} className="flex items-center gap-2 p-2 rounded-lg hover:bg-white cursor-pointer font-mono text-xs">
                      <input type="checkbox" checked={selectedUnitsToReserve.includes(unit.unitId)} onChange={e => {
                    if (e.target.checked) {
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setReserveModalReq(null)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold cursor-pointer">
                Cancel
              </button>
              <button onClick={handleConfirmReserve} className="px-5 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-black shadow-md cursor-pointer">
                Confirm Reservation
              </button>
            </div>
          </div>
      ...
      {issueModalReq && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-sky-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <select value={selectedUnitIdToIssue} onChange={e => setSelectedUnitIdToIssue(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold font-mono">
                  <option value="">Auto-select matching unit from vault</option>
                  {bloodUnitsList
                .filter(u => u.bloodGroup === issueModalReq.bloodGroup)
                .map(u => (<option key={u.unitId} value={u.unitId}>
              ...
              <div>
                ...
                <input type="text" value={receivingPartyInput} onChange={e => setReceivingPartyInput(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold" required/>
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setIssueModalReq(null)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold cursor-pointer">
                Cancel
              </button>
              <button onClick={handleConfirmIssue} className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black shadow-md shadow-emerald-500/20 cursor-pointer">
                Confirm & Issue Blood
              </button>
            </div>
          </div>
      ...
      {showIntakeModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-md bg-white border border-sky-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl">
            <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <form onSubmit={handleIntakeSubmit} className="space-y-3">
              <div>
                ...
                <select value={intakeGroup} onChange={e => setIntakeGroup(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
                  {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', 'Bombay Phenotype (O-h)'].map(bg => (<option key={bg} value={bg}>{bg}</option>))}
                </select>
              </div>

              <div>
                <label className="text-slate-700 font-bold block mb-1">Blood Component *</label>
                <select value={intakeComp} onChange={e => setIntakeComp(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold">
              ...
              <div>
                ...
                <input type="text" value={intakeStorageLoc} onChange={e => setIntakeStorageLoc(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold" required/>
              </div>

              <div>
                <label className="text-slate-700 font-bold block mb-1">Donor Reference / Source</label>
                <input type="text" value={intakeDonorRef} onChange={e => setIntakeDonorRef(e.target.value)} className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold"/>
              </div>

              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setShowIntakeModal(false)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-700 font-bold cursor-pointer">
                  Cancel
                </button>
                <button type="submit" className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black shadow-md shadow-emerald-500/20 cursor-pointer">
                  Record Intake Unit
                </button>
              </div>
            </form>
