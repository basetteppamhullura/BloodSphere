...
import { Lock, X, Send, MapPin, Building2, Clock, Calendar, CheckCircle2, ShieldCheck, Check, CheckCheck, Droplet } from 'lucide-react';
export const EmergencyChatModal = () => {
    const { activeChatSessionId, chatSessions, closeEmergencyChatModal, sendChatMessage, markChatMessagesRead, closeEmergencyChatSession, setTypingIndicator, requests } = useApp();
    ...
    const roleTitle = userRole === 'hospital'
        ? `Hospital ↔ Requester`
        : userRole === 'bloodbank'
    ...
    const participantName = userRole === 'hospital' || userRole === 'bloodbank'
    ...
    const handleSendText = (e) => {
        e.preventDefault();
        if (!inputText.trim() || isClosed || !currentUser)
            return;
        sendChatMessage({
    ...
    const handleQuickAction = (type, textPayload, metadata) => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-2xl h-[90vh] max-h-[750px] bg-white border border-sky-100 rounded-3xl overflow-hidden shadow-2xl flex flex-col relative">
        ...
        <div className="p-4 border-b border-sky-100 bg-gradient-to-r from-sky-50 via-white to-red-50 flex items-center justify-between gap-3 relative z-10 shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={closeEmergencyChatModal} className="p-2 rounded-xl bg-sky-50 text-slate-400 hover:text-slate-700 hover:bg-sky-100 transition-colors" title="Close Chat Window">
              <X className="w-5 h-5"/>
            </button>
          </div>
        </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-3.5 relative z-10 text-xs bg-slate-50/50">
          ...
          {activeSession.messages.map(msg => {
            ...
            return (<div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} space-y-1 animate-in fade-in`}>
                <span className="text-[9px] text-slate-400 font-bold px-1">
                  {isMe ? 'You' : msg.senderName} • {msg.timestamp}
                </span>

                <div className={`max-w-[85%] sm:max-w-[75%] p-3.5 rounded-2xl shadow-2xs space-y-1.5 ${isMe
                  ...
                  {msg.messageType === 'text' && (<p className="text-xs leading-relaxed whitespace-pre-wrap">{msg.message}</p>)}

                  {msg.messageType === 'hospital_location' && (<div className="space-y-1.5">
                      <div className="flex items-center gap-1.5 font-extrabold text-xs">
                        <MapPin className="w-4 h-4 shrink-0"/> Hospital Geolocation Shared
                      </div>
                      <p className="text-[11px] opacity-95">{msg.message}</p>
                      {msg.metadata?.location && (<div className="p-2 rounded-xl bg-black/10 text-[10px] font-mono">
                          📍 Lat: {msg.metadata.location.lat.toFixed(4)}, Lng: {msg.metadata.location.lng.toFixed(4)}
                        </div>)}
                    </div>)}

                  {msg.messageType === 'hospital_details' && (<div className="space-y-1.5">
                      <div className="flex items-center gap-1.5 font-extrabold text-xs">
                  ...
                  {msg.messageType === 'eta' && (<div className="space-y-1">
                      <div className="flex items-center gap-1.5 font-extrabold text-xs">
                  ...
                  {msg.messageType === 'appointment_details' && (<div className="space-y-1">
                      <div className="flex items-center gap-1.5 font-extrabold text-xs">
                  ...
                  {msg.messageType === 'confirm_availability' && (<div className="space-y-1">
                      <div className="flex items-center gap-1.5 font-extrabold text-xs">
                  ...
                  <div className="flex items-center justify-end gap-1 text-[9px] opacity-80 pt-0.5 font-mono">
                    {msg.read ? (<span className="flex items-center gap-0.5 text-emerald-200">
          ...
          {/* Typing Indicator */}
          {isOtherUserTyping && (<div className="flex items-center gap-2 text-[11px] text-slate-500 font-medium italic animate-pulse">
              <span className="w-2 h-2 rounded-full bg-sky-500 animate-ping"/>
              <span>{isUserDonor ? activeSession.requesterName : activeSession.donorName} is typing...</span>
            </div>)}

          {/* Read Only Closed Banner */}
          {isClosed && (<div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-center space-y-1 font-bold text-xs my-3">
        ...
        {!isClosed && (<div className="px-4 py-2 bg-white border-t border-sky-100 flex items-center gap-2 overflow-x-auto text-[11px] font-bold relative z-10 shrink-0">
            <button onClick={() => handleQuickAction('hospital_location', `📍 Hospital Location: ${activeSession.hospitalName}, ${activeSession.hospitalAddress || 'Vidyanagar, Hubballi'}`)} className="px-3 py-1.5 rounded-full bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 shrink-0 flex items-center gap-1 transition-colors">
              <MapPin className="w-3.5 h-3.5 text-red-500"/> Share Hospital Location
            </button>

            <button onClick={() => handleQuickAction('eta', `⏱ Donor ETA: Arriving at ${activeSession.hospitalName} within 15–20 minutes.`)} className="px-3 py-1.5 rounded-full bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 shrink-0 flex items-center gap-1 transition-colors">
              <Clock className="w-3.5 h-3.5 text-amber-500"/> ETA 15 Mins
            </button>

            <button onClick={() => handleQuickAction('confirm_availability', `✅ Availability Confirmed: Ready for immediate donor screening & blood collection.`)} className="px-3 py-1.5 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 shrink-0 flex items-center gap-1 transition-colors">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600"/> Confirm Availability
            </button>

            <button onClick={() => handleQuickAction('hospital_details', `🏥 Hospital Details: ${activeSession.hospitalName} • Emergency Trauma Ward Bed 14.`)} className="px-3 py-1.5 rounded-full bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 shrink-0 flex items-center gap-1 transition-colors">
        ...
        <div className="p-4 border-t border-sky-100 bg-white relative z-10 shrink-0">
          {!isClosed ? (<form onSubmit={handleSendText} className="flex items-center gap-2">
              <input type="text" placeholder="Type a message..." value={inputText} onChange={handleInputChange} className="flex-1 p-3 rounded-2xl bg-slate-50 border border-sky-200 text-slate-900 font-medium text-xs focus:border-red-500 focus:outline-none shadow-2xs"/>

              <button type="submit" disabled={!inputText.trim()} className="px-5 py-3 rounded-2xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 disabled:opacity-50 text-white font-extrabold text-xs shadow-md shadow-red-500/20 flex items-center gap-1.5 transition-all hover:scale-105">
                <Send className="w-4 h-4"/> Send
              </button>

              <button type="button" onClick={() => closeEmergencyChatSession(activeSession.id)} className="px-3 py-3 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-[10px] transition-colors" title="Complete & Close Emergency Chat">
                End Chat
              </button>
            </form>) : (<div className="text-center py-1 text-[11px] text-slate-400 font-mono">
