      ...
      <div className="p-1.5 rounded-2xl bg-white border border-sky-100 shadow-xs flex items-center gap-1 overflow-x-auto font-extrabold">
        <button
          ...
          className={`px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 shrink-0 ${
            activeTab === 'overview' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 hover:bg-sky-50'
          }`}
        >
          <Activity className="w-4 h-4" /> Control Overview
        </button>

        <button
          onClick={() => { setActiveTab('users'); setRoleFilter('ALL'); }}
          className={`px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 shrink-0 ${
            activeTab === 'users' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 hover:bg-sky-50'
          }`}
        >
          <Users className="w-4 h-4" /> All Accounts & Approvals ({totalRegisteredUsers})
        </button>

        <button
          onClick={() => { setActiveTab('donors'); setRoleFilter('donor'); }}
          className={`px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 shrink-0 ${
            activeTab === 'donors' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 hover:bg-sky-50'
          }`}
        >
          <Heart className="w-4 h-4 text-red-500" /> Donors ({totalDonorsCount})
        </button>

        <button
          onClick={() => { setActiveTab('requesters'); setRoleFilter('requester'); }}
          className={`px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 shrink-0 ${
            activeTab === 'requesters' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 hover:bg-sky-50'
          }`}
        >
          <FileText className="w-4 h-4" /> Requesters ({totalRequestersCount})
        </button>

        <button
          onClick={() => setActiveTab('hospitals')}
          className={`px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 shrink-0 relative ${
            activeTab === 'hospitals' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 hover:bg-sky-50'
          }`}
        >
          <Building2 className="w-4 h-4 text-sky-600" /> Hospitals Verification
        ...
        <button
          ...
          className={`px-4 py-2.5 rounded-xl transition-all flex items-center gap-1.5 shrink-0 relative ${
            activeTab === 'bloodbanks' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 hover:bg-sky-50'
          }`}
        >
          <Droplet className="w-4 h-4 text-red-600" /> Blood Banks Verification
          {pendingBloodBanksCount > 0 && (
            <span className="px-1.5 py-0.5 rounded-full text-[9px] font-black bg-red-600 text-white animate-pulse">
