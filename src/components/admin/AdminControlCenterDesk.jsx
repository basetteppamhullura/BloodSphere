...
import { ShieldCheck, Users, Building2, FileText, ShieldAlert, AlertTriangle, Heart, BarChart3, Settings, Check, Ban, Lock, Activity, Eye, X, Landmark, Boxes, Clock } from 'lucide-react';
export const AdminControlCenterDesk = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const { requests, donors, bloodBanks, inventoryStockMap, bloodUnitsList, activityLogs, notifications, chatSessions, showToast, toggleDonorAvailability, markDonationCompleted } = useApp();
    const { portalAccounts, updateAccountStatusByAdmin, deleteAccountByAdmin, currentUser } = useAuth();
    ...
    const getActiveTabFromPath = () => {
        ...
        if (path.includes('/admin/live-activity') || path.includes('/admin/activity'))
            return 'activity';
        if (path.includes('/admin/requests'))
            return 'requests';
        if (path.includes('/admin/accounts') || path.includes('/admin/users'))
            return 'accounts';
        if (path.includes('/admin/requesters'))
            return 'requesters';
        if (path.includes('/admin/donors'))
            return 'donors';
        if (path.includes('/admin/hospitals'))
            return 'hospitals';
        if (path.includes('/admin/blood-banks') || path.includes('/admin/bloodbanks'))
            return 'bloodbanks';
        if (path.includes('/admin/inventory') || path.includes('/admin/stock'))
            return 'inventory';
        if (path.includes('/admin/analytics') || path.includes('/admin/reports'))
            return 'analytics';
    ...
    const [activityPortalFilter, setActivityPortalFilter] = useState('ALL');
    // Modal States
    ...
    const [selectedDonorModal, setSelectedDonorModal] = useState(null);
    const [selectedHospitalDocsModal, setSelectedHospitalDocsModal] = useState(null);
    const [rejectingAccountModal, setRejectingAccountModal] = useState(null);
    const [rejectionReasonText, setRejectionReasonText] = useState('Registration details or clinical license proof is incomplete.');
    // Audit Logs State
    ...
    const logAdminAction = (action, targetEntity, details, status = 'SUCCESS') => {
        const newEntry = {
            id: `AUD-${Date.now()}`,
    ...
    if (!isSuperAdmin) {
        return (<div className="p-12 rounded-3xl bg-white border border-red-200 shadow-sm text-center space-y-4 max-w-lg mx-auto my-12">
        ...
        <button onClick={() => navigate('/login/admin')} className="px-6 py-3 rounded-2xl bg-red-600 text-white font-extrabold text-xs shadow-md shadow-red-500/20 cursor-pointer">
    ...
    const totalAvailableUnits = Object.values(inventoryStockMap).reduce((acc, row) => acc + Object.values(row).reduce((a, b) => a + (b.available || 0), 0), 0);
    ...
    const liveActivityEvents = [
        ...activityLogs.map((log) => ({
            ...
            statusType: log.action.includes('ISSUED') ? 'urgent' : 'info'
        })),
        ...requests.map((r) => ({
            id: `act_${r.id}`,
            ...
            requestId: r.id,
            statusType: r.urgency === 'CRITICAL' ? 'urgent' : 'success'
        }))
    ].sort((a, b) => (b.timestamp > a.timestamp ? 1 : -1));
    // --- ACCOUNT ACTION HANDLERS ---
    const handleApproveAccount = (acc) => {
    ...
    const handleConfirmRejection = (e) => {
        e.preventDefault();
        if (!rejectingAccountModal)
            return;
        updateAccountStatusByAdmin(rejectingAccountModal.id, 'Disabled');
        logAdminAction('ACCOUNT_REJECTED', `${rejectingAccountModal.name} (${rejectingAccountModal.role.toUpperCase()})`, `Rejected registration request. Reason: "${rejectionReasonText.trim()}".`, 'WARNING');
        showToast(`Rejected registration for ${rejectingAccountModal.name}.`);
        setRejectingAccountModal(null);
    };
    const handleSuspendAccount = (acc) => {
    ...
    const handleReactivateAccount = (acc) => {
        updateAccountStatusByAdmin(acc.id, 'Verified');
    ...
    const filteredDonors = donors.filter(d => {
        if (bloodGroupFilter !== 'ALL' && d.bloodGroup !== bloodGroupFilter)
            return false;
        if (searchQuery.trim()) {
            const q = searchQuery.toLowerCase().trim();
    ...
    const filteredRequests = requests.filter(r => {
        ...
        if (urgencyFilter !== 'ALL' && r.urgency !== urgencyFilter)
            return false;
        if (bloodGroupFilter !== 'ALL' && r.bloodGroup !== bloodGroupFilter)
            return false;
        if (searchQuery.trim()) {
            const q = searchQuery.toLowerCase().trim();
    ...
    const filteredLiveActivities = liveActivityEvents.filter(act => {
        if (activityPortalFilter !== 'ALL' && act.portal !== activityPortalFilter)
            return false;
        if (searchQuery.trim()) {
            const q = searchQuery.toLowerCase().trim();
    ...
    return (<div className="space-y-6 text-xs animate-in fade-in max-w-6xl mx-auto pb-16">
      ...
      {activeTab === 'overview' && (<div className="space-y-6">
          ...
          {lowStockGroups.length > 0 && (<div className="p-4 rounded-2xl bg-red-50 border border-red-200 text-red-900 space-y-2">
              ...
              <div className="flex flex-wrap gap-2 pt-1 font-mono">
                {lowStockGroups.map(([grp, comps]) => {
                    ...
                    return (<span key={grp} className="px-3 py-1 rounded-xl bg-red-600 text-white font-extrabold text-[10px]">
      ...
      {/* ========================================================================= */}
      {activeTab === 'activity' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-5 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input type="text" placeholder="Search activity, actor, request..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>
              <select value={activityPortalFilter} onChange={e => setActivityPortalFilter(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs cursor-pointer">
          ...
          <div className="space-y-2">
            {filteredLiveActivities.map((act) => (<div key={act.id} className="p-4 rounded-2xl bg-slate-50/80 border border-slate-200/80 hover:bg-white hover:border-amber-300 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-start gap-3">
                  <span className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm shrink-0 font-bold ${act.portal === 'REQUESTER'
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {act.requestId && (<span className="px-2 py-0.5 rounded-md text-[9px] font-mono font-bold bg-amber-100 text-amber-800">
                          ID: {act.requestId}
                        </span>)}
                    </div>
      ...
      {/* ========================================================================= */}
      {activeTab === 'accounts' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input type="text" placeholder="Search name, email, city..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>

              <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {filteredAccounts.map(acc => (<tr key={acc.id} className="hover:bg-sky-50/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${acc.status === 'Verified'
                    ...
                    <td className="py-3.5 px-4 font-sans flex items-center gap-1.5">
                      {acc.status !== 'Verified' && (<button onClick={() => handleApproveAccount(acc)} className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-[10px] shadow-xs flex items-center gap-1 cursor-pointer">
                          <Check className="w-3.5 h-3.5"/> Approve
                        </button>)}
                      {acc.status === 'Pending Verification' && (<button onClick={() => setRejectingAccountModal(acc)} className="px-3 py-1.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-extrabold text-[10px] shadow-xs flex items-center gap-1 cursor-pointer">
                          <Ban className="w-3.5 h-3.5"/> Reject
                        </button>)}
                      {acc.status === 'Verified' ? (<button onClick={() => handleSuspendAccount(acc)} className="px-2.5 py-1.5 rounded-xl bg-slate-100 hover:bg-amber-100 text-slate-700 hover:text-amber-800 font-extrabold text-[10px] transition-colors cursor-pointer">
                          Suspend
                        </button>) : acc.status === 'Disabled' ? (<button onClick={() => handleReactivateAccount(acc)} className="px-2.5 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-800 font-extrabold text-[10px] transition-colors cursor-pointer">
                          Reactivate
                        </button>) : null}
                    </td>
                  </tr>))}
      ...
      {/* ========================================================================= */}
      {activeTab === 'requesters' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <input type="text" placeholder="Search requesters..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredRequesters.map(reqUser => {
                ...
                return (<div key={reqUser.id} className="p-5 rounded-3xl bg-white border border-slate-200 shadow-xs space-y-3">
                  ...
                  <button onClick={() => {
                        ...
                        if (matched)
                            setSelectedRequestModal(matched);
      ...
      {/* ========================================================================= */}
      {activeTab === 'donors' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input type="text" placeholder="Search donor name or city..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>
              <select value={bloodGroupFilter} onChange={e => setBloodGroupFilter(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {filteredDonors.map(donor => (<tr key={donor.id} className="hover:bg-sky-50/40 transition-colors">
                    <td className="py-3.5 px-4 font-sans font-extrabold text-slate-900">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${donor.isAvailable !== false
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <button onClick={() => setSelectedDonorModal(donor)} className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-[10px] cursor-pointer">
                        Inspect Details
                      </button>
                    </td>
                  </tr>))}
      ...
      {/* ========================================================================= */}
      {activeTab === 'hospitals' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <input type="text" placeholder="Search hospital or license..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredHospitals.map(hosp => {
                const hospRequests = requests.filter(r => r.hospitalName.toLowerCase().includes(hosp.name.toLowerCase()));
                return (<div key={hosp.id} className="p-5 rounded-3xl bg-white border border-slate-200 shadow-xs space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${hosp.status === 'Verified' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                      {hosp.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    {hosp.status !== 'Verified' && (<button onClick={() => handleApproveAccount(hosp)} className="flex-1 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs shadow-xs cursor-pointer">
                        Approve License Verification
                      </button>)}
                    <button onClick={() => showToast(`Opening live stock triage for ${hosp.name}...`)} className="flex-1 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs cursor-pointer">
      ...
      {/* ========================================================================= */}
      {activeTab === 'bloodbanks' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <input type="text" placeholder="Search blood bank or license..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredBloodBanks.map(bb => (<div key={bb.id} className="p-5 rounded-3xl bg-white border border-slate-200 shadow-xs space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${bb.status === 'Verified' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>
                    {bb.status}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-1">
                  {bb.status !== 'Verified' && (<button onClick={() => handleApproveAccount(bb)} className="flex-1 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold text-xs shadow-xs cursor-pointer">
                      Approve Regulatory License
                    </button>)}
                  <button onClick={() => navigate('/admin/inventory')} className="flex-1 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs cursor-pointer">
      ...
      {/* ========================================================================= */}
      {activeTab === 'requests' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input type="text" placeholder="Search Request ID, Patient..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs w-48"/>

              <select value={urgencyFilter} onChange={e => setUrgencyFilter(e.target.value)} className="p-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {filteredRequests.map(req => (<tr key={req.id} className="hover:bg-sky-50/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${req.urgency === 'CRITICAL' ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800'}`}>
                        {req.urgency}
                      </span>
                    </td>
                    <td className="py-3.5 px-4 font-sans text-[10px]">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center justify-between gap-1.5">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${(req.channelStatuses?.donorStatus || 'PENDING') === 'APPROVED'
                    ? 'bg-emerald-100 text-emerald-800'
                    : (req.channelStatuses?.donorStatus || 'PENDING') === 'REJECTED'
                        ...
                        <div className="flex items-center justify-between gap-1.5">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${(req.channelStatuses?.hospitalStatus || 'PENDING') === 'APPROVED'
                    ? 'bg-emerald-100 text-emerald-800'
                    : (req.channelStatuses?.hospitalStatus || 'PENDING') === 'REJECTED'
                        ...
                        <div className="flex items-center justify-between gap-1.5">
                          ...
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase ${(req.channelStatuses?.bloodBankStatus || 'PENDING') === 'APPROVED'
                    ? 'bg-emerald-100 text-emerald-800'
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <button onClick={() => setSelectedRequestModal(req)} className="px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-[10px] flex items-center gap-1 cursor-pointer">
      ...
      {/* ========================================================================= */}
      {activeTab === 'inventory' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700">
              ...
              <tbody className="divide-y divide-sky-100 font-mono">
                {bloodUnitsList.map(unit => (<tr key={unit.unitId} className="hover:bg-sky-50/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-sans">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${unit.status === 'STORED'
                    ? 'bg-emerald-100 text-emerald-800'
      ...
      {/* ========================================================================= */}
      {activeTab === 'audit-logs' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex items-center justify-between border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          <div className="space-y-2.5 font-mono">
            {auditLogs.map(log => (<div key={log.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex items-start justify-between gap-3 text-xs">
                <div>
                  <div className="flex items-center gap-2">
                ...
                <div className="text-right shrink-0">
                  ...
                  <span className={`text-[10px] font-black uppercase ${log.status === 'SUCCESS' ? 'text-emerald-700' : 'text-amber-700'}`}>
                    {log.status}
                  </span>
                </div>
              </div>))}
      ...
      {/* ========================================================================= */}
      {activeTab === 'settings' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-6 shadow-sm">
          <div className="flex items-center justify-between border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          <div className="space-y-4 max-w-lg">
            <div>
              ...
              <input type="number" value={systemSettings.lowStockThreshold} onChange={e => setSystemSettings(prev => ({ ...prev, lowStockThreshold: parseInt(e.target.value) || 1 }))} className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 font-bold text-xs"/>
            </div>

            <div>
              <label className="text-slate-700 font-extrabold block mb-1">Donor Donation Cooldown (Days)</label>
              <input type="number" value={systemSettings.donorCooldownDays} onChange={e => setSystemSettings(prev => ({ ...prev, donorCooldownDays: parseInt(e.target.value) || 90 }))} className="w-full p-3 rounded-2xl bg-slate-50 border border-slate-200 font-bold text-xs"/>
            </div>
      ...
      {/* ========================================================================= */}
      {selectedRequestModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="bg-white rounded-3xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-5 shadow-2xl border border-sky-100 animate-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-sky-100 pb-3">
              ...
              <button onClick={() => setSelectedRequestModal(null)} className="p-1 rounded-xl text-slate-400 hover:text-slate-700">
                <X className="w-5 h-5"/>
              </button>
            </div>
              ...
              <div className="grid grid-cols-3 gap-2.5">
                ...
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${(selectedRequestModal.channelStatuses?.donorStatus || 'PENDING') === 'APPROVED'
                ? 'bg-emerald-100 text-emerald-800'
                : (selectedRequestModal.channelStatuses?.donorStatus || 'PENDING') === 'REJECTED'
                  ...
                  {selectedRequestModal.channelStatuses?.donorRejectionReason && (<div className="text-[9px] text-red-600 font-sans">
                ...
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${(selectedRequestModal.channelStatuses?.hospitalStatus || 'PENDING') === 'APPROVED'
                ? 'bg-emerald-100 text-emerald-800'
                : (selectedRequestModal.channelStatuses?.hospitalStatus || 'PENDING') === 'REJECTED'
                  ...
                  {selectedRequestModal.channelStatuses?.hospitalRejectionReason && (<div className="text-[9px] text-red-600 font-sans">
                ...
                <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${(selectedRequestModal.channelStatuses?.bloodBankStatus || 'PENDING') === 'APPROVED'
                ? 'bg-emerald-100 text-emerald-800'
                : (selectedRequestModal.channelStatuses?.bloodBankStatus || 'PENDING') === 'REJECTED'
                  ...
                  {selectedRequestModal.channelStatuses?.bloodBankRejectionReason && (<div className="text-[9px] text-red-600 font-sans">
            ...
            <div className="space-y-3">
              ...
              <div className="relative pl-6 space-y-4 border-l-2 border-sky-200 font-mono">
                ...
                {selectedRequestModal.channelStatuses?.hospitalRespondedAt && (<div className="relative">
                    <span className={`absolute -left-[31px] top-1 w-3.5 h-3.5 rounded-full ring-4 ring-white ${selectedRequestModal.channelStatuses.hospitalStatus === 'APPROVED' ? 'bg-emerald-500' : 'bg-red-500'}`}/>
                ...
                {selectedRequestModal.channelStatuses?.bloodBankRespondedAt && (<div className="relative">
                    <span className={`absolute -left-[31px] top-1 w-3.5 h-3.5 rounded-full ring-4 ring-white ${selectedRequestModal.channelStatuses.bloodBankStatus === 'APPROVED' ? 'bg-emerald-500' : 'bg-red-500'}`}/>
                ...
                {selectedRequestModal.channelStatuses?.donorRespondedAt && (<div className="relative">
                    <span className={`absolute -left-[31px] top-1 w-3.5 h-3.5 rounded-full ring-4 ring-white ${selectedRequestModal.channelStatuses.donorStatus === 'APPROVED' ? 'bg-emerald-500' : 'bg-red-500'}`}/>
            ...
            <div className="pt-3 border-t border-sky-100 flex justify-end">
              <button onClick={() => setSelectedRequestModal(null)} className="px-5 py-2.5 rounded-2xl bg-slate-900 text-white font-extrabold text-xs cursor-pointer">
                Close Timeline View
              </button>
            </div>
          </div>
      ...
      {/* ========================================================================= */}
      {rejectingAccountModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <form onSubmit={handleConfirmRejection} className="bg-white rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl border border-red-200">
            ...
            <textarea value={rejectionReasonText} onChange={e => setRejectionReasonText(e.target.value)} rows={3} className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs" required/>
            <div className="flex items-center justify-end gap-2">
              <button type="button" onClick={() => setRejectingAccountModal(null)} className="px-4 py-2 rounded-xl text-slate-500 hover:bg-slate-100 font-bold text-xs cursor-pointer">
                Cancel
              </button>
              <button type="submit" className="px-4 py-2 rounded-xl bg-red-600 text-white font-extrabold text-xs shadow-sm cursor-pointer">
                Confirm Rejection
              </button>
            </div>
          </form>
      ...
      {/* ========================================================================= */}
      {selectedDonorModal && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl border border-sky-100">
            ...
            <div className="pt-2 flex justify-end">
              <button onClick={() => setSelectedDonorModal(null)} className="px-4 py-2 rounded-xl bg-slate-900 text-white font-bold text-xs cursor-pointer">
                Close
              </button>
            </div>
          </div>
