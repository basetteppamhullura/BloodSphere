...
import { Activity, X, CheckCircle2, Droplet } from 'lucide-react';
export const HealthPassportModal = () => {
    const { activeHealthPassportModal, setActiveHealthPassportModal, showToast } = useApp();
    const { currentUser } = useAuth();
    if (!activeHealthPassportModal)
        return null;
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
      <div className="w-full max-w-xl max-h-[85vh] bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-xs flex-1">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              {currentUser?.hbTrendHistory?.map((item, i) => (<div key={i} className="p-3 rounded-xl bg-slate-950 border border-slate-800">
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950 flex justify-end">
          <button onClick={() => { showToast("Health Passport updated!"); setActiveHealthPassportModal(false); }} className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs">
            Save Health Log
          </button>
        </div>
