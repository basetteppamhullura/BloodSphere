    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-lg bg-white border border-sky-100 rounded-3xl p-6 space-y-4 text-xs shadow-2xl relative flex flex-col h-[520px]">
        ...
        <div className="flex items-center justify-between border-b border-sky-100 pb-3 shrink-0">
          ...
          <button onClick={() => setActiveChatModal(null)} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors">
            <X className="w-4 h-4"/>
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 p-3 rounded-2xl bg-sky-50/50 border border-sky-100 text-xs">
          {chatHistory.map((msg, i) => (<div key={i} className={`p-3 rounded-2xl max-w-[85%] space-y-1 ${msg.sender === 'System'
        ...
        <form onSubmit={handleSendMessage} className="flex items-center gap-2 pt-2 shrink-0">
          <input type="text" placeholder="Type encrypted message..." value={messageText} onChange={e => setMessageText(e.target.value)} className="flex-1 p-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-bold text-xs focus:outline-none focus:border-sky-500"/>
          <button type="submit" className="p-3 rounded-xl bg-red-600 hover:bg-red-500 text-white font-extrabold shadow-md shadow-red-500/20">
