...
import { AlertTriangle, X, Send, ShieldCheck, Building2, MapPin, User, FileText, Clock, Upload, CheckCircle2, ChevronRight, ChevronLeft, Crosshair, Edit2, FileCheck, KeyRound, Check, Share2, Droplet, Users } from 'lucide-react';
export const EmergencyPostModal = () => {
    const { activeEmergencyPostModal, setActiveEmergencyPostModal, createEmergencyRequest, bloodBanks, verifyPatientCredentials } = useApp();
    const [currentStep, setCurrentStep] = useState(1);
    const [errors, setErrors] = useState({});
    // Multi-Channel Selection State (Requires min 1)
    const [selectedChannels, setSelectedChannels] = useState(['hospital', 'donors']);
    ...
    const [verificationFeedback, setVerificationFeedback] = useState('Patient verified successfully.');
    // 1. Patient Info
    ...
    const [reasonCategory, setReasonCategory] = useState('Surgery');
    ...
    const [gpsCoords, setGpsCoords] = useState({ lat: 15.3688, lng: 75.1274 });
    ...
    const handleHospitalSelect = (hName) => {
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) {
            ...
            if (!patientIdInput.trim())
                newErrors.patientIdInput = 'Patient ID is required.';
            if (!verificationCodeInput.trim())
                newErrors.verificationCodeInput = 'Verification code is required.';
        }
        if (step === 2) {
            if (unitsNeeded < 1)
                newErrors.unitsNeeded = 'Minimum 1 unit required.';
            if (reasonCategory === 'Other' && !customReason.trim())
                newErrors.customReason = 'Please specify reason.';
        }
        if (step === 3) {
            if (!selectedHospital.trim())
                newErrors.selectedHospital = 'Hospital name is required.';
            if (!city.trim())
        ...
        if (step === 6) {
            ...
            if (!/^\d{10}$/.test(requesterPhone.trim()))
                newErrors.requesterPhone = 'Valid 10-digit mobile number required.';
            if (requesterEmail && !/\S+@\S+\.\S+/.test(requesterEmail))
                newErrors.requesterEmail = 'Invalid email address.';
        }
        if (step === 7) {
            if (!city.trim())
                newErrors.city = 'City is required.';
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
      <div className="w-full max-w-2xl max-h-[92vh] bg-white border border-sky-100 rounded-3xl overflow-hidden shadow-2xl flex flex-col relative">
        ...
        <div className="p-6 overflow-y-auto space-y-4 text-xs flex-1 relative z-10">
          ...
          {currentStep === 1 && (<div className="space-y-4 animate-in fade-in">
              ...
              <div className="p-4 rounded-2xl bg-sky-50/50 border border-sky-100 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" placeholder="BN-HUB-2026-00852" value={patientIdInput} onChange={e => setPatientIdInput(e.target.value)} className="w-full p-2.5 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 font-mono shadow-2xs"/>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input type="text" placeholder="739241" value={verificationCodeInput} onChange={e => setVerificationCodeInput(e.target.value)} className="w-full p-2.5 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 font-mono shadow-2xs"/>
                      <button type="button" onClick={handleVerifyCredentials} className="px-4 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-black shrink-0 flex items-center gap-1 shadow-xs">
                ...
                {verificationFeedback && (<div className={`p-2.5 rounded-xl border text-xs flex items-center gap-2 font-bold ${isVerified
              ...
              <div className="space-y-3 pt-1">
                <div>
                  ...
                  <input type="text" placeholder="e.g. Rohan Deshmukh" value={patientName} onChange={e => setPatientName(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" placeholder="e.g. 34" value={patientAge} onChange={e => setPatientAge(e.target.value ? Number(e.target.value) : '')} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                  ...
                  <div>
                    ...
                    <select value={patientGender} onChange={e => setPatientGender(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs">
          ...
          {currentStep === 2 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={bloodGroup} onChange={e => setBloodGroup(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs font-bold">
                ...
                <div>
                  ...
                  <input type="number" min={1} max={10} value={unitsNeeded} onChange={e => setUnitsNeeded(Number(e.target.value))} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={bloodComponent} onChange={e => setBloodComponent(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs">
                ...
                <div>
                  ...
                  <select value={reasonCategory} onChange={e => setReasonCategory(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs">
              ...
              {reasonCategory === 'Other' && (<div>
                  <label className="text-slate-700 font-bold block mb-1">Specify Custom Reason *</label>
                  <input type="text" placeholder="Specify medical condition..." value={customReason} onChange={e => setCustomReason(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                  {errors.customReason && <span className="text-[10px] text-red-500 block mt-1">{errors.customReason}</span>}
                </div>)}
            </div>)}

          {/* STEP 3: HOSPITAL DETAILS */}
          {currentStep === 3 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" list="registered-hospitals" placeholder="Search or type hospital..." value={selectedHospital} onChange={e => handleHospitalSelect(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                ...
                <div>
                  ...
                  <input type="text" value={hospitalAddress} onChange={e => setHospitalAddress(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-slate-700 font-bold block mb-1">Department / Ward</label>
                    <input type="text" placeholder="e.g. ICU Bed 14" value={wardDept} onChange={e => setWardDept(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                  </div>

                  <div>
                    <label className="text-slate-700 font-bold block mb-1">Hospital Phone *</label>
                    <input type="text" value={hospitalPhone} onChange={e => setHospitalPhone(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
          ...
          {currentStep === 4 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div onClick={() => setUrgency('CRITICAL')} className={`p-4 rounded-2xl border cursor-pointer text-center space-y-1 transition-all ${urgency === 'CRITICAL'
                ...
                : 'bg-white border-sky-100 text-slate-600 hover:border-sky-200'}`}>
                  ...
                  <div onClick={() => setUrgency('HIGH')} className={`p-4 rounded-2xl border cursor-pointer text-center space-y-1 transition-all ${urgency === 'HIGH'
                ...
                : 'bg-white border-sky-100 text-slate-600 hover:border-sky-200'}`}>
                  ...
                  <div onClick={() => setUrgency('MODERATE')} className={`p-4 rounded-2xl border cursor-pointer text-center space-y-1 transition-all ${urgency === 'MODERATE'
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div>
                  ...
                  <input type="date" min={new Date().toISOString().split('T')[0]} value={requiredDate} onChange={e => setRequiredDate(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                ...
                <div>
                  ...
                  <input type="text" placeholder="e.g. 10:30 AM" value={requiredTime} onChange={e => setRequiredTime(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
          ...
          {currentStep === 5 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              {errors.channels && (<div className="p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 font-bold text-xs">
                  {errors.channels}
                </div>)}

              <div className="space-y-3">
                {/* Hospital Channel */}
                <div onClick={() => toggleChannel('hospital')} className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-start gap-3 ${selectedChannels.includes('hospital')
                ? 'bg-sky-50/80 border-sky-400 text-slate-900 ring-1 ring-sky-300'
                : 'bg-white border-sky-100 text-slate-600 hover:border-sky-200'}`}>
                  <div className="mt-0.5">
                    <input type="checkbox" checked={selectedChannels.includes('hospital')} onChange={() => { }} className="w-4 h-4 rounded accent-sky-600"/>
                  </div>
                  <div>
                ...
                <div onClick={() => toggleChannel('donors')} className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-start gap-3 ${selectedChannels.includes('donors')
                ? 'bg-red-50/80 border-red-300 text-slate-900 ring-1 ring-red-300'
                : 'bg-white border-sky-100 text-slate-600 hover:border-sky-200'}`}>
                  <div className="mt-0.5">
                    <input type="checkbox" checked={selectedChannels.includes('donors')} onChange={() => { }} className="w-4 h-4 rounded accent-red-600"/>
                  </div>
                  <div>
                    <span className="font-extrabold text-slate-900 text-xs flex items-center gap-1.5">
                ...
                {/* Blood Banks Channel */}
                <div onClick={() => toggleChannel('bloodbank')} className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-start gap-3 ${selectedChannels.includes('bloodbank')
                ? 'bg-emerald-50/80 border-emerald-300 text-slate-900 ring-1 ring-emerald-300'
                : 'bg-white border-sky-100 text-slate-600 hover:border-sky-200'}`}>
                  <div className="mt-0.5">
                    <input type="checkbox" checked={selectedChannels.includes('bloodbank')} onChange={() => { }} className="w-4 h-4 rounded accent-emerald-600"/>
                  </div>
                  <div>
                    <span className="font-extrabold text-slate-900 text-xs flex items-center gap-1.5">
          ...
          {currentStep === 6 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={requesterName} onChange={e => setRequesterName(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                ...
                <div>
                  ...
                  <select value={relationship} onChange={e => setRelationship(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" placeholder="9876543210" value={requesterPhone} onChange={e => setRequesterPhone(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                ...
                <div>
                  ...
                  <input type="email" value={requesterEmail} onChange={e => setRequesterEmail(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
          ...
          {/* STEP 7: LOCATION & GPS */}
          {currentStep === 7 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={stateName} onChange={e => setStateName(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs">
                ...
                <div>
                  ...
                  <select value={district} onChange={e => setDistrict(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={city} onChange={e => setCity(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                ...
                <div>
                  ...
                  <input type="text" placeholder="580031" value={pincode} onChange={e => setPincode(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
              ...
              <div className="p-4 rounded-2xl bg-sky-50/70 border border-sky-100 flex items-center justify-between">
                ...
                <button type="button" onClick={handleUseMyLocation} disabled={isLocating} className="px-4 py-2 rounded-xl bg-white hover:bg-sky-50 text-sky-700 font-bold text-xs border border-sky-200 flex items-center gap-1.5 shadow-2xs">
          ...
          {currentStep === 8 && (<div className="space-y-4 animate-in fade-in">
              <h4 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" placeholder="e.g. Dr. Mahesh Kulkarni" value={doctorName} onChange={e => setDoctorName(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                </div>

                <div>
                  <label className="text-slate-700 font-bold block mb-1">Medical Notes & Condition Summary</label>
                  <textarea rows={3} placeholder="Additional details regarding patient condition..." value={medicalNotes} onChange={e => setMedicalNotes(e.target.value)} className="w-full p-3 rounded-xl bg-white border border-sky-200 text-slate-900 focus:border-red-500 focus:outline-none shadow-2xs"/>
                </div>
                ...
                <div>
                  ...
                  <div className="p-4 rounded-2xl border-2 border-dashed border-sky-200 bg-sky-50/50 text-center space-y-2">
                    {prescriptionFile ? (<div className="flex items-center justify-between p-3 rounded-xl bg-white border border-sky-200 text-xs">
                        <div className="flex items-center gap-2">
                          <FileCheck className="w-5 h-5 text-emerald-600"/>
                          <span className="font-bold text-slate-800 truncate max-w-xs">{prescriptionFile.name}</span>
                        </div>
                        <button type="button" onClick={() => setPrescriptionFile(null)} className="text-red-600 font-bold hover:underline">
                          Remove
                        </button>
                      </div>) : (<label className="cursor-pointer block space-y-1">
                        ...
                        <input type="file" accept="image/*,.pdf" onChange={e => e.target.files?.[0] && setPrescriptionFile(e.target.files[0])} className="hidden"/>
                      </label>)}
                  </div>
                </div>
              </div>
          ...
          {currentStep === 9 && (<div className="space-y-4 animate-in fade-in">
        ...
        <div className="p-4 border-t border-sky-100 bg-gradient-to-r from-sky-50 via-white to-blue-50 flex items-center justify-between relative z-10">
          {currentStep > 1 ? (<button type="button" onClick={handlePrevStep} className="px-4 py-2 rounded-xl bg-white hover:bg-sky-50 text-slate-700 font-bold text-xs border border-sky-200 flex items-center gap-1 shadow-2xs">
              <ChevronLeft className="w-4 h-4"/> Previous
            </button>) : <div />}

          {currentStep < 9 ? (<button type="button" onClick={handleNextStep} className="px-6 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white font-extrabold text-xs shadow-md shadow-red-500/20 flex items-center gap-1 transition-all">
              Next Section <ChevronRight className="w-4 h-4"/>
            </button>) : (<button type="button" onClick={handleFinalSubmit} className="px-8 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white font-black text-xs shadow-lg shadow-red-500/25 flex items-center gap-2 transition-all hover:scale-105">
