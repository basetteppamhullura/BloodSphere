...
import { ShieldAlert, Award, CheckCircle2, Clock, MapPin, Building2, MessageSquare, Zap, Calendar, AlertCircle, Check, X, ShieldCheck, Trophy, Activity, FileText, Download } from 'lucide-react';
export const RealtimeDonorPortal = () => {
    const { requests, donors, donorRespondToRequest, toggleDonorAvailability, openEmergencyChat, leaderboard, scheduleDonationAppointment, showToast } = useApp();
    ...
    const incomingRequests = requests.filter(r => {
        if (r.status === 'COMPLETED' || r.status === 'CANCELLED' || r.status === 'EXPIRED')
            return false;
    ...
    return (<div className="space-y-6 text-xs animate-in fade-in max-w-6xl mx-auto pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-sky-100 pb-4">
          ...
          <div className="p-3 rounded-2xl bg-sky-50/80 border border-sky-200 flex items-center gap-3">
            <div>
              ...
              <select value={loggedInDonor.availabilityStatus || 'AVAILABLE'} onChange={e => toggleDonorAvailability(loggedInDonor.id, e.target.value, loggedInDonor.emergencyAlertsEnabled)} className="bg-white text-slate-900 font-extrabold text-xs p-1.5 rounded-xl border border-sky-200 focus:outline-none cursor-pointer">
      ...
      {incomingRequests.some(r => r.urgency === 'CRITICAL') && (<div className="p-4 rounded-3xl bg-gradient-to-r from-red-600 to-rose-700 text-white shadow-lg flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in">
          ...
          <button onClick={() => setActiveTab('requests')} className="px-5 py-2.5 rounded-2xl bg-white text-red-700 font-black text-xs hover:bg-red-50 shadow-md shrink-0 transition-all hover:scale-105">
            Respond Now
          </button>
        </div>)}
      ...
      <div className="p-1.5 rounded-2xl bg-slate-100 border border-slate-200 flex flex-wrap gap-2 text-xs font-extrabold">
        <button onClick={() => setActiveTab('requests')} className={`flex-1 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${activeTab === 'requests'
        ...
        <button onClick={() => setActiveTab('tracking')} className={`flex-1 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${activeTab === 'tracking'
            ? 'bg-red-600 text-white font-black shadow-sm'
            : 'text-slate-700 hover:bg-slate-200/60'}`}>
          <Activity className="w-4 h-4"/> <span>Request Tracking ({acceptedRequests.length})</span>
        </button>

        <button onClick={() => setActiveTab('appointments')} className={`flex-1 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${activeTab === 'appointments'
            ? 'bg-red-600 text-white font-black shadow-sm'
            : 'text-slate-700 hover:bg-slate-200/60'}`}>
          <Calendar className="w-4 h-4"/> <span>Appointments ({appointments.length})</span>
        </button>

        <button onClick={() => setActiveTab('impact')} className={`flex-1 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${activeTab === 'impact'
            ? 'bg-red-600 text-white font-black shadow-sm'
            : 'text-slate-700 hover:bg-slate-200/60'}`}>
          <Trophy className="w-4 h-4"/> <span>Impact & Badges</span>
        </button>

        <button onClick={() => setActiveTab('history')} className={`flex-1 px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${activeTab === 'history'
            ? 'bg-red-600 text-white font-black shadow-sm'
            : 'text-slate-700 hover:bg-slate-200/60'}`}>
      ...
      {activeTab === 'requests' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          ...
          {incomingRequests.length === 0 ? (<div className="p-10 rounded-2xl bg-sky-50/40 border border-sky-100 text-center text-slate-500 space-y-2">
              ...
              {incomingRequests.map(req => {
                    ...
                    return (<div key={req.id} className={`p-5 rounded-3xl border-2 transition-all space-y-4 shadow-sm ${req.urgency === 'CRITICAL' ? 'border-red-300 bg-red-50/30 ring-1 ring-red-300' : 'border-sky-100 bg-white'}`}>
                    <div className="flex items-start justify-between gap-3 border-b border-sky-100 pb-3">
                      ...
                      {(() => {
                            ...
                            return (<span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1 border ${donorStatus === 'APPROVED'
                    ...
                    <div className="pt-1">
                      {isAccepted || req.channelStatuses?.donorStatus === 'APPROVED' ? (<div className="p-3 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center justify-between">
                          <span>🟢 Response: "I Can Donate" (Approved)</span>
                          <button onClick={() => openEmergencyChat(req.id, loggedInDonor.id)} className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 text-white text-[11px] font-extrabold flex items-center gap-1 shadow-xs transition-all hover:scale-105 cursor-pointer">
                            <MessageSquare className="w-3.5 h-3.5"/> 💬 Open Chat
                          </button>
                        </div>) : isDeclined || req.channelStatuses?.donorStatus === 'REJECTED' ? (<div className="p-3 rounded-2xl bg-red-50 border border-red-200 text-red-700 text-xs font-bold text-center">
      ...
      {activeTab === 'tracking' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          ...
          {acceptedRequests.length === 0 ? (<div className="p-10 rounded-2xl bg-slate-50 border border-slate-200 text-center text-slate-500 space-y-2">
              ...
              {acceptedRequests.map(req => (<div key={req.id} className="p-5 rounded-3xl bg-white border border-indigo-100 space-y-4 shadow-xs">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
                    <div>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => openEmergencyChat(req.id, loggedInDonor.id)} className="px-4 py-2 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 text-white text-xs font-extrabold flex items-center gap-1.5 shadow-sm">
      ...
      {activeTab === 'appointments' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {appointments.map(apt => (<div key={apt.id} className="p-5 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-xs">
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <button onClick={() => showToast(`Appointment ${apt.id} verified at ${apt.centerName}`)} className="px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-extrabold text-xs shadow-xs">
      ...
      {activeTab === 'impact' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-6 shadow-sm">
      ...
      {activeTab === 'history' && (<div className="p-6 rounded-3xl bg-white border border-sky-100 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3">
            <div>
              <h3 className="font-extrabold text-base text-slate-900 flex items-center gap-2">
          ...
          <div className="divide-y divide-sky-100">
            {donationHistory.map(item => (<div key={item.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button onClick={() => showToast(`Downloaded Official Donation Certificate ${item.certId}`)} className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs border border-slate-200 flex items-center gap-1.5 shrink-0">
