...
import { LayoutDashboard, AlertTriangle, ShieldCheck, Users, Search, Trophy, User, Sun, Moon, ChevronRight, Heart } from 'lucide-react';
export const DonorSidebar = () => {
    ...
    return (<aside className="hidden lg:flex flex-col w-64 shrink-0 bg-white border border-sky-100 rounded-3xl p-5 shadow-xs sticky top-20 h-fit space-y-5 text-slate-800">
      ...
      <div className="flex items-center justify-between border-b border-sky-100 pb-3">
        ...
        <span className={`flex items-center gap-1 text-[10px] font-black px-2 py-0.5 rounded-full border ${connectionStatus === 'ONLINE'
        ...
        {navItems.map(item => {
            ...
            return (<NavLink key={item.to} to={item.to} className={({ isActive }) => `w-full flex items-center justify-between px-3.5 py-3 rounded-2xl transition-all ${isActive
              ...
              {({ isActive }) => (<>
                  ...
                  {item.badge && item.badge > 0 ? (<span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isActive ? 'bg-white text-red-600' : 'bg-red-100 text-red-700 font-mono'}`}>
                      {item.badge}
