...
import { Home, LayoutDashboard, AlertTriangle, ShieldCheck, Users, Search, Trophy, User, ChevronRight, Sun, Moon, X } from 'lucide-react';
export const Sidebar = () => {
    const { activePage, navigateTo, requests, connectionStatus, isMobileSidebarOpen, setIsMobileSidebarOpen } = useApp();
    ...
    const renderVerticalMenu = () => (<nav className="w-full flex flex-col space-y-3 font-sans">
      {donorNavItems.map(item => {
            const IconComp = item.icon;
            const isActive = activePage === item.tab;
            const isEmergency = item.tab === 'emergency-requests';
            return (<button key={item.tab} onClick={() => handleNavClick(item.tab)} className={`w-full flex items-center justify-between px-3.5 py-3 rounded-2xl text-xs font-extrabold transition-all duration-200 cursor-pointer ${isActive
                    ? isEmergency
            ...
            <div className="flex items-center gap-3 min-w-0">
              <IconComp className={`w-4 h-4 shrink-0 ${isActive ? 'text-white' : isEmergency ? 'text-red-600' : 'text-sky-600'}`}/>
              <span className="truncate tracking-tight">{item.label}</span>
            </div>

            {/* Right: Badge / Status Arrow */}
            <div className="flex items-center gap-1.5 shrink-0 ml-2">
              {isEmergency && criticalCount > 0 && (<span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase ${isActive
    ...
    return (<>
      ...
      <aside className="hidden lg:flex flex-col w-64 shrink-0 bg-white border border-sky-100 rounded-3xl p-5 shadow-sm sticky top-20 h-fit space-y-5 text-slate-800">
        ...
        <div className="flex items-center justify-between border-b border-sky-100 pb-3">
          ...
          <span className={`flex items-center gap-1.5 text-[10px] font-black px-2.5 py-1 rounded-full border ${connectionStatus === 'ONLINE'
            ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
            : connectionStatus === 'RECONNECTING'
                ? 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse'
                : 'bg-red-50 text-red-700 border-red-200'}`} title="Real-Time Network Status">
            <span className={`w-2 h-2 rounded-full ${connectionStatus === 'ONLINE'
        ...
        <div className="pt-3 border-t border-sky-100 flex items-center justify-between text-xs text-slate-500 font-medium">
          ...
          <button onClick={toggleTheme} className="p-1.5 rounded-xl bg-sky-50 hover:bg-sky-100 text-slate-600 border border-sky-100 transition-all" title="Toggle Light/Dark Theme">
      ...
      {/* ================================================== */}
      {isMobileSidebarOpen && (<div className="lg:hidden fixed inset-0 z-50 flex animate-in fade-in">
          {/* Backdrop Overlay */}
          <div onClick={() => setIsMobileSidebarOpen(false)} className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs transition-opacity"/>
          ...
          <div className="relative w-72 max-w-[85vw] bg-white h-full p-5 flex flex-col justify-between shadow-2xl z-10 border-r border-sky-100 overflow-y-auto space-y-5">
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between border-b border-sky-100 pb-3">
                ...
                <button onClick={() => setIsMobileSidebarOpen(false)} className="p-1.5 rounded-xl bg-slate-100 text-slate-600 hover:text-slate-900">
                  <X className="w-5 h-5"/>
                </button>
              </div>
            ...
            <div className="pt-3 border-t border-sky-100 flex items-center justify-between text-xs text-slate-500 font-medium">
              ...
              <button onClick={toggleTheme} className="p-1.5 rounded-xl bg-sky-50 text-slate-600 border border-sky-100">
                {isDarkMode ? <Sun className="w-3.5 h-3.5 text-amber-500"/> : <Moon className="w-3.5 h-3.5 text-sky-600"/>}
              </button>
            </div>
