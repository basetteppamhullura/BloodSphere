...
import { ShieldCheck, Users, Building2, FileText, BarChart3, Settings, Heart, ChevronRight, ShieldAlert, Activity, Boxes, Clock, Landmark } from 'lucide-react';
    ...
    return (<>
      ...
      <aside className="hidden lg:flex flex-col w-64 shrink-0 bg-white border border-sky-100 rounded-3xl p-5 shadow-xs sticky top-20 h-fit space-y-5 text-slate-800">
        ...
        <nav className="w-full flex flex-col space-y-5 text-xs font-extrabold">
          {navSections.map(section => (<div key={section.title} className="space-y-1.5">
              ...
              <div className="space-y-1">
                ...
                return (<NavLink key={item.to} to={item.to} onClick={() => setIsMobileSidebarOpen(false)} className={({ isActive }) => `w-full flex items-center justify-between px-3.5 py-2.5 rounded-2xl transition-all ${isActive
                      ...
                      {({ isActive }) => (<>
                          ...
                          {item.badge && item.badge > 0 ? (<span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isActive ? 'bg-white text-amber-600' : 'bg-amber-100 text-amber-800'}`}>
                              {item.badge}
      ...
      {isMobileSidebarOpen && (<div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          <div className="relative w-4/5 max-w-xs bg-white h-full shadow-2xl p-5 overflow-y-auto flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-sky-100 pb-3">
                ...
                <button onClick={() => setIsMobileSidebarOpen(false)} className="p-1 rounded-xl text-slate-400 hover:text-slate-700">
                  ✕
                </button>
              </div>
              ...
              <nav className="space-y-4 text-xs font-extrabold">
                {navSections.map(section => (<div key={section.title} className="space-y-1">
                    ...
                    {section.items.map(item => {
                    const IconComp = item.icon;
                    return (<NavLink key={item.to} to={item.to} onClick={() => setIsMobileSidebarOpen(false)} className={({ isActive }) => `w-full flex items-center justify-between px-3 py-2.5 rounded-xl ${isActive ? 'bg-amber-600 text-white font-black' : 'text-slate-700 hover:bg-amber-50'}`}>
                          <div className="flex items-center gap-2">
                            <IconComp className="w-4 h-4"/>
                            <span>{item.label}</span>
                          </div>
                          {item.badge && item.badge > 0 && (<span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-amber-100 text-amber-800">
                              {item.badge}
                            </span>)}
                        </NavLink>);
                })}
