...
import { Home, LayoutDashboard, AlertTriangle, User, Menu, Droplet, Building2, ShieldCheck } from 'lucide-react';
export const MobileNav = () => {
    ...
    return (<div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-sky-100 px-2 py-2 flex items-center justify-around text-slate-500 shadow-lg">
      
      {/* Menu Toggle Drawer Button */}
      <button onClick={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)} className={`flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition-all ${isMobileSidebarOpen ? 'text-sky-600 font-extrabold bg-sky-50' : 'hover:text-slate-900'}`}>
      ...
      {roleNavItems.map(item => {
            ...
            return (<NavLink key={item.to} to={item.to} onClick={() => setIsMobileSidebarOpen(false)} className={({ isActive }) => `flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl transition-all ${isActive
