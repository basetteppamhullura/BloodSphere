...
export const HospitalHeaderNav = () => {
    ...
    return (<header className="sticky top-0 z-40 bg-white border-b border-[#DDE8E2] shadow-xs">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4 border-b border-[#DDE8E2]/60">
        ...
        <div className="flex items-center gap-3 text-xs">
          {activeChats.length > 0 && (<button onClick={() => openEmergencyChat(activeChats[0].requestId)} className="px-3 py-1.5 rounded-xl bg-red-50 text-red-600 border border-red-200 font-extrabold flex items-center gap-1.5 hover:bg-red-100 transition-colors">
              <MessageSquare className="w-3.5 h-3.5 text-red-600"/>
              <span className="hidden sm:inline">Emergency Chat</span>
            </button>)}

          <button onClick={() => setActiveEmergencyPostModal(true)} className="hidden md:flex px-3.5 py-1.5 rounded-xl bg-[#087443] hover:bg-[#065b34] text-white font-extrabold text-xs shadow-xs items-center gap-1.5 transition-all">
            <PlusCircle className="w-3.5 h-3.5"/> Post Trauma Need
          </button>

          {currentUser && (<div className="flex items-center gap-2 pl-2 border-l border-[#DDE8E2]">
              <span className="font-bold text-xs text-[#18352A] hidden lg:inline">{currentUser.name}</span>
              <button onClick={handleLogout} className="p-1.5 rounded-xl text-[#587067] hover:text-red-600 hover:bg-red-50 transition-all flex items-center gap-1 font-bold text-xs" title="Logout">
          ...
          <nav className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5 text-xs font-extrabold whitespace-nowrap scroll-smooth">
            ...
            return (<NavLink key={item.to} to={item.to} className={({ isActive }) => `flex items-center gap-2 px-3.5 py-2 rounded-xl transition-all shrink-0 ${isActive
                  ...
                  {({ isActive }) => (<>
                      ...
                      {item.badge && item.badge > 0 ? (<span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${isActive ? 'bg-white text-[#087443]' : 'bg-red-100 text-red-700 font-mono'}`}>
                          {item.badge}
                        </span>) : null}
                    </>)}
                </NavLink>);
