...
import { LayoutDashboard, PlusCircle, AlertTriangle, Search, Bell, User, Sun, Moon, ChevronRight, LifeBuoy } from 'lucide-react';
export const RequesterSidebar = () => {
        ...
        {navItems.map(item => {
            ...
            return (<NavLink key={item.to} to={item.to} className={({ isActive }) => `w-full flex items-center justify-between px-3.5 py-3 rounded-2xl transition-all ${isActive
              ...
              {({ isActive }) => (<>
                  ...
                  {item.badge && item.badge > 0 ? (<span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${isActive ? 'bg-white text-sky-600' : 'bg-sky-100 text-sky-800'}`}>
                      {item.badge}
