...
import { Home, PlusCircle, User, Building2, Droplet, Lock, LogOut, ChevronDown, Menu, MessageSquare, Shield, Users, Heart } from 'lucide-react';
export const Header = () => {
    ...
    const unreadChatCount = activeChats.reduce((sum, s) => {
        if (currentRole === 'requester')
            return sum + (s.unreadCountRequester || 0);
        if (currentRole === 'donor')
            return sum + (s.unreadCountDonor || 0);
    ...
    return (<header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-sky-100/90 shadow-2xs relative overflow-visible">
      ...
      <div className="absolute inset-0 pointer-events-none select-none overflow-hidden opacity-30">
        ...
        <svg className="absolute bottom-0 left-0 right-0 w-full h-4 text-sky-200/40" preserveAspectRatio="none" viewBox="0 0 1200 40" fill="none">
          <path d="M0,20 C300,5 600,35 900,15 C1050,5 1150,25 1200,20 L1200,40 L0,40 Z" fill="currentColor"/>
        </svg>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[72px] sm:min-h-[80px] py-2.5 flex items-center justify-between gap-3 relative z-10">
        ...
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          ...
          <button type="button" onClick={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)} className="lg:hidden p-2 rounded-xl bg-sky-50 hover:bg-sky-100 text-slate-700 border border-sky-100 flex items-center justify-center transition-colors cursor-pointer" title="Toggle Menu" aria-label="Toggle navigation menu">
        ...
        <nav className="hidden xl:flex items-center gap-1 text-xs font-bold text-slate-700">
          <NavLink to="/" className={({ isActive }) => `px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-colors cursor-pointer ${isActive
          ...
          <Link to="/login/donor" className="px-3 py-1.5 rounded-full flex items-center gap-1.5 hover:bg-red-50/70 text-slate-700 hover:text-red-600 transition-colors cursor-pointer">
            <Heart className="w-3.5 h-3.5 text-red-600"/>
            <span>Donor Portal</span>
          </Link>

          <Link to="/login/requester" className="px-3 py-1.5 rounded-full flex items-center gap-1.5 hover:bg-rose-50/70 text-slate-700 hover:text-rose-600 transition-colors cursor-pointer">
            <Users className="w-3.5 h-3.5 text-rose-600"/>
            <span>Requester Portal</span>
          </Link>

          <Link to="/login/hospital" className="px-3 py-1.5 rounded-full flex items-center gap-1.5 hover:bg-sky-50/70 text-slate-700 hover:text-sky-600 transition-colors cursor-pointer">
            <Building2 className="w-3.5 h-3.5 text-sky-600"/>
            <span>Hospital Portal</span>
          </Link>

          <Link to="/login/bloodbank" className="px-3 py-1.5 rounded-full flex items-center gap-1.5 hover:bg-emerald-50/70 text-slate-700 hover:text-emerald-600 transition-colors cursor-pointer">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          <button type="button" onClick={handleEmergencyChatClick} className="px-3 py-2 rounded-full bg-white hover:bg-red-50/60 text-red-600 border border-red-200 hover:border-red-300 font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs hover:scale-102 shrink-0" title="Open Private Real-Time Emergency Chat">
            <MessageSquare className="w-4 h-4 text-red-600 shrink-0"/>
            <span className="hidden sm:inline font-bold">Emergency Chat</span>
            {chatBadgeCount > 0 && (<span className="min-w-4.5 h-4.5 px-1.5 rounded-full bg-red-600 text-white text-[10px] font-black flex items-center justify-center shadow-xs animate-pulse">
                {chatBadgeCount}
              </span>)}
          </button>

          {/* 3.2 POST EMERGENCY NEED */}
          <button type="button" onClick={() => setActiveEmergencyPostModal(true)} className="px-3.5 sm:px-4 py-2 rounded-full bg-[#DC2626] hover:bg-[#B91C1C] text-white font-bold text-xs shadow-sm shadow-red-500/20 flex items-center gap-1.5 transition-all cursor-pointer hover:scale-102 active:scale-98 shrink-0" title="Create an emergency blood request">
          ...
          <div className="relative shrink-0" ref={dropdownRef}>
            <button type="button" onClick={() => setIsLoginDropdownOpen(!isLoginDropdownOpen)} className="px-3 sm:px-3.5 py-2 rounded-full bg-[#F0F9FF] hover:bg-[#E0F2FE] text-[#0369A1] border border-[#BAE6FD] font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-2xs" aria-expanded={isLoginDropdownOpen} aria-haspopup="true">
              <User className="w-3.5 h-3.5 text-[#0284C7] shrink-0"/>
              <span className="hidden md:inline">Portal Access</span>
              <ChevronDown className={`w-3.5 h-3.5 text-[#0284C7] transition-transform duration-200 ${isLoginDropdownOpen ? 'rotate-180' : ''}`}/>
            </button>

            {/* Dropdown Menu */}
            {isLoginDropdownOpen && (<div className="absolute right-0 mt-2.5 w-64 rounded-2xl bg-white border border-sky-100 shadow-xl p-2 z-50 space-y-1 text-xs animate-in fade-in slide-in-from-top-2 duration-150">
                ...
                <Link to="/login/donor" onClick={() => setIsLoginDropdownOpen(false)} className="p-2.5 rounded-xl hover:bg-red-50/60 cursor-pointer flex items-center justify-between text-slate-800 transition-colors group">
                ...
                <Link to="/login/requester" onClick={() => setIsLoginDropdownOpen(false)} className="p-2.5 rounded-xl hover:bg-rose-50/60 cursor-pointer flex items-center justify-between text-slate-800 transition-colors group">
                  <div className="flex items-center gap-2.5">
                ...
                <Link to="/login/hospital" onClick={() => setIsLoginDropdownOpen(false)} className="p-2.5 rounded-xl hover:bg-sky-50/60 cursor-pointer flex items-center justify-between text-slate-800 transition-colors group">
                  <div className="flex items-center gap-2.5">
                ...
                <Link to="/login/bloodbank" onClick={() => setIsLoginDropdownOpen(false)} className="p-2.5 rounded-xl hover:bg-emerald-50/60 cursor-pointer flex items-center justify-between text-slate-800 transition-colors group">
                  <div className="flex items-center gap-2.5">
                ...
                <Link to="/login/admin" onClick={() => setIsLoginDropdownOpen(false)} className="p-2.5 rounded-xl hover:bg-amber-50/60 cursor-pointer flex items-center justify-between text-slate-800 transition-colors border-t border-slate-100 group">
                  <div className="flex items-center gap-2.5">
          ...
          {currentUser && (<>
              ...
              {/* Logout */}
              <button type="button" onClick={handleLogout} className="px-2.5 py-1.5 rounded-full text-slate-600 hover:text-red-600 hover:bg-red-50/80 transition-all flex items-center gap-1.5 cursor-pointer font-bold text-xs shrink-0" title="Logout from BloodNet" aria-label="Logout">
