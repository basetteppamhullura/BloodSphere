...
import { MOCK_EMERGENCY_REQUESTS, MOCK_DONORS, MOCK_BLOOD_BANKS, MOCK_CAMPS, MOCK_LEADERBOARD, MOCK_NOTIFICATIONS, MOCK_GROUP_CIRCLES, MOCK_INTERCITY_TRANSFERS, MOCK_PATIENT_VERIFICATIONS } from '../data/mockData.js';
...
const INITIAL_INVENTORY_MATRIX = {
    'A+': {
...
export const AppProvider = ({ children }) => {
    ...
    // Modals
    const [activeSmartMatchModal, setActiveSmartMatchModal] = useState(null);
    const [activeEmergencyPostModal, setActiveEmergencyPostModal] = useState(false);
    ...
    const [activeHealthPassportModal, setActiveHealthPassportModal] = useState(false);
    const [activeCorporateImpactModal, setActiveCorporateImpactModal] = useState(false);
    ...
    useEffect(() => {
        ...
        const handleNewEmergencyRequest = (data) => {
            if (data?.request) {
                setRequests(prev => {
    ...
    const createOrGetEmergencyChatSession = (requestId, donorId) => {
    ...
    const openEmergencyChat = (requestId, donorId) => {
    ...
    const markChatMessagesRead = (sessionId, userId) => {
        setChatSessions(prev => prev.map(s => {
            if (s.id === sessionId) {
    ...
    const closeEmergencyChatSession = (sessionId) => {
    ...
    const setTypingIndicator = (sessionId, userId, isTyping) => {
        const session = chatSessions.find(s => s.id === sessionId);
        if (session) {
            socketManager.emitTyping(session.requestId, userId, isTyping);
        }
    };
    const createPatientVerification = (patientName, bloodGroup, hospitalName) => {
    ...
    const verifyPatientCredentials = (patientId, code, bloodGroup) => {
        const match = patientVerifications.find(v => v.patientId.toLowerCase().trim() === patientId.toLowerCase().trim() && v.isActive);
        if (!match) {
    ...
    const detectDuplicateRequests = (patientId, group, hospitalName) => {
    ...
    const sendDirectRequestToDonor = (requestId, donorId) => {
    ...
    const respondToEmergencyRequest = (requestId, donorId, responseStatus) => {
        const donorObj = donors.find(d => d.id === donorId) || donors[0];
        const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        setRequests(prev => prev.map(req => {
            if (req.id === requestId) {
                ...
                const updatedTimeline = (req.requestTimeline || DEFAULT_TIMELINE).map(step => {
                    if (step.id === 'step_response_received') {
                        return { ...step, status: 'completed', timestamp: timeStr, description: `${acceptedCount} donor(s) responded` };
                    }
                    if (step.id === 'step_blood_reserved' && isFullySecured) {
                        return { ...step, status: 'completed', timestamp: timeStr, description: `Required ${req.unitsNeeded} units confirmed & secured!` };
                    }
                    return step;
                });
    ...
    const toggleDonorAvailability = (donorId, status, emergencyAlerts) => {
        setDonors(prev => prev.map(d => {
            if (d.id === donorId) {
                return {
                    ...d,
    ...
    const reserveBloodBankUnits = (requestId, bankIdOrGroup, groupOrUnits, componentOrStaff, unitsParam, staffNameParam) => {
        ...
        if (typeof groupOrUnits === 'number') {
            ...
            group = bankIdOrGroup;
            units = groupOrUnits;
            component = req?.bloodComponent || 'PRBC';
            staffName = componentOrStaff || 'Rotary Blood Bank Staff';
        }
        else if (typeof groupOrUnits === 'string') {
            // Called as (requestId, bankId, group, component, units, staffName)
            group = groupOrUnits;
            component = componentOrStaff || req?.bloodComponent || 'PRBC';
            units = unitsParam || 1;
        ...
        let unitsMarked = 0;
        setBloodUnitsList(prev => prev.map(u => {
            if (unitsMarked < units && u.bloodGroup === group && (u.component === component || !u.component) && (u.status === 'STORED' || u.status === 'APPROVED' || u.status === 'COLLECTED')) {
                unitsMarked++;
                return { ...u, status: 'RESERVED', lastUpdated: new Date().toISOString().split('T')[0] };
            }
            return u;
        }));
        const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    ...
    const issueBloodBankUnits = (requestId, unitIdOrGroup, staffNameOrUnits, selectedUnitIdParam, receivingPartyParam) => {
        let unitId = '';
        ...
        const bg = unit?.bloodGroup || 'O+';
        const comp = unit?.component || 'PRBC';
        // 1. Update Blood Unit Lifecycle Status to ISSUED
        setBloodUnitsList(prev => prev.map(u => (u.unitId === unitId ? { ...u, status: 'ISSUED', lastUpdated: new Date().toISOString().split('T')[0] } : u)));
    ...
    const acceptBloodRequest = (requestId, centerName) => {
        const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    ...
    const rejectBloodRequest = (requestId, centerName, reason) => {
    ...
    const rejectBloodBankRequest = (requestId, reason) => {
    ...
    const redirectBloodRequest = (requestId, fromCenter, targetCenter, targetCity, reason) => {
        const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        setRequests(prev => prev.map(req => {
            if (req.id === requestId) {
                const timeline = req.requestTimeline || [];
    ...
    const intakeBloodUnit = (unitData, staffName) => {
    ...
    const approveBloodBankReservation = (requestId, bankId) => {
        const req = requests.find(r => r.id === requestId);
        if (req) {
            reserveBloodBankUnits(requestId, bankId, req.bloodGroup, req.bloodComponent || 'PRBC', req.unitsNeeded, 'Regional Admin');
        }
    };
    const approveRequestByHospital = (requestId, notes) => {
        const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        setRequests(prev => prev.map(r => {
            if (r.id === requestId) {
                return {
    ...
    const rejectRequestByHospital = (requestId, reason) => {
        const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    ...
    const donorAcceptRequest = (requestId, donorId) => {
        respondToEmergencyRequest(requestId, donorId, 'ACCEPTED');
        openEmergencyChat(requestId, donorId);
    };
    const donorDeclineRequest = (requestId, donorId) => {
        respondToEmergencyRequest(requestId, donorId, 'DECLINED');
    };
    const scheduleDonationAppointment = (requestId, date, time, venue) => {
        setRequests(prev => prev.map(r => {
            if (r.id === requestId) {
                return {
                    ...r,
    ...
    const createInterCityTransfer = (data) => {
        const newTrf = {
    ...
    const updateInterCityTransferStatus = (id, status) => {
        setInterCityTransfers(prev => prev.map(t => (t.id === id ? { ...t, status, courierStatus: status } : t)));
        const newLog = {
            activityId: `ACT-${Date.now()}`,
    ...
    const markDonationCompleted = (requestId, donorId) => {
        setRequests(prev => prev.map(r => {
            if (r.id === requestId) {
                return {
                    ...r,
    ...
    const updateInventoryStock = (bankId, group, change) => {
